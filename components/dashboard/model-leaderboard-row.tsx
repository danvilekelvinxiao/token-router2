...
export default function ModelLeaderboardRow({ item, showTooltip = false }: ModelLeaderboardRowProps) {
  ...
  const tokenLabel = item.tokensLabel || formatTokens(item.tokens);
  ...
  return (
    <div className={`model-leaderboard-row ${item.rank <= 3 ? "is-top" : ""}`}>
      ...
      <div className="model-leaderboard-main">
        ...
        <div className="model-leaderboard-provider" title={provider}>by {provider}</div>
      </div>
      <div className="model-leaderboard-metric">
        <div className="model-leaderboard-token">{tokenLabel} <span>tokens</span></div>
      ...
      {hasTooltip ? (
        <div className="model-leaderboard-tooltip" role="tooltip">
          ...
          <span>请求次数：{formatNumber(item.requests)} 次</span>
          <span>消耗金额：¥{Number(item.costCny || 0).toFixed(2)}</span>
