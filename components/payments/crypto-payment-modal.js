      ...
      <div className="payment-modal-shell payment-crypto-modal" role="dialog" aria-modal="true" aria-label={title}>
        ...
        <div className="payment-modal-summary-grid crypto">
          <div className="payment-modal-amount-card">
            <span>应付数量</span>
            <div className="payment-modal-amount-copy-row">
              <strong>{amountLabel}</strong>
              <button type="button" className="btn-secondary" onClick={() => onCopy(String(amountLabel || "").split(" ")[0])}>
                {copied === String(amountLabel || "").split(" ")[0] ? "已复制" : "复制数量"}
              </button>
            </div>
            <small>{usdAmountLabel ? `约合 ${usdAmountLabel} · ` : ""}订单金额 {cnyAmountLabel}</small>
          </div>
