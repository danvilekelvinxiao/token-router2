...
import { getNewApiAdminConfigState } from "@/lib/new-api/admin-auth.mjs";
...
export default async function handler(req, res) {
  ...
  const newApiAdminState = getNewApiAdminConfigState();
  const checks = [
    item("管理员鉴权", "normal", `当前管理员：${admin.customer?.email || admin.customer?.id || "admin"}`),
    item("激活码创建接口", checkActivationPayloads() && fileExists("pages/api/admin/activation-codes.js") ? "normal" : "abnormal", "支持金额、Token 余额、套餐服务三类 dry-run 校验。"),
    ...
    item("New API 连接", process.env.NEW_API_BASE_URL || newApiAdminState.hasToken || newApiAdminState.hasCredentials || process.env.NEW_API_KEY ? "normal" : "未配置", "检查 New API 环境变量。"),
