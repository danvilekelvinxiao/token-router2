...
export default function AdminUsers() {
  ...
  const formatLimit = (key) => {
    ...
    return limit.unit === "token" ? `${used} / ${amount} Token 额度` : `已用 $ API ${used} / 限制 $ API ${amount}`;
  };
      ...
      <AdminLayout currentPath="/admin/users">
        <div style={{ color: "var(--dash-text)" }}>
          <header style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
            <div>
              <h1 style={{ fontSize: 24, fontWeight: 900, margin: 0 }}>用户与调用限制</h1>
              <p style={{ fontSize: 13, color: "var(--dash-sub)", margin: "4px 0 0" }}>管理用户账号、账户余额、API Key 权限和调用限制</p>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => fetchUsers()} style={{ padding: "8px 14px", borderRadius: 7, border: "1px solid var(--dash-accent)", background: "transparent", color: "var(--dash-accent)", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>刷新</button>
            </div>
          </header>
                    ...
                    <tbody>
                      {users.map((u) => (
                        <tr key={u.id} style={{ borderTop: "1px solid var(--dash-border)", background: selectedUser?.id === u.id ? "var(--dash-card-hover)" : "transparent" }}>
                          ...
                          <td style={{ ...tdS, fontFamily: "'SF Mono', monospace" }}>{formatApiMoney(u.balance || 0)}</td>
                          <td style={tdS}>{u.keyCount || 0}</td>
                          <td style={tdS}>{u.todayReqs || 0}</td>
                          <td style={{ ...tdS, fontFamily: "'SF Mono', monospace" }}>
                            {formatTokens(u.todayTokens || 0)} / {formatApiMoneyPrecise(u.todaySpend || 0)}
                          </td>
              ...
              {editMode && selectedUser && (
                <div style={{ background: "var(--dash-card-bg)", border: "1px solid var(--dash-border)", borderRadius: 10, padding: "22px 24px" }}>
                  ...
                  <div style={{ display: "grid", gap: 12 }}>
                    ...
                    <EditField label="$ API 账户余额" value={String(editForm.balance || 0)} onChange={(v) => setEditForm({ ...editForm, balance: Number(v) })} type="number" />
                  </div>

                  <div style={{ marginTop: 18, paddingTop: 16, borderTop: "1px solid var(--dash-border)" }}>
                    <h4 style={{ margin: "0 0 10px", fontSize: 13, fontWeight: 900 }}>API Key 限制</h4>
                      ...
                      {(selectedUser.apiKeys || []).map((key) => (
                        <div key={key.id} style={{ display: "grid", gap: 8, padding: 10, border: "1px solid var(--dash-border)", borderRadius: 8, background: "var(--dash-card-hover)", minWidth: 0 }}>
                          <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "flex-start" }}>
                            ...
                            <button type="button" onClick={() => openKeyLimitEdit(key)} style={btnSmStyle}>改限制</button>
                          </div>
                    ...
                    {keyLimitEdit && (
                      <div style={{ display: "grid", gap: 10, marginTop: 12, padding: 12, border: "1px solid var(--dash-accent)", borderRadius: 8, background: "var(--dash-card-bg)" }}>
                        <label style={adminLimitLabelStyle}>限制类型
                        ...
                        {keyLimitForm.type !== "none" && (
                          <>
                            <label style={adminLimitLabelStyle}>限制上限
                            ...
                            <label style={adminLimitLabelStyle}>单位
                              <select value={keyLimitForm.unit} onChange={(e) => setKeyLimitForm({ ...keyLimitForm, unit: e.target.value })} style={adminLimitInputStyle}>
                                <option value="cny">$ API 余额限制</option>
                        ...
                        <div style={{ display: "flex", gap: 8 }}>
                          <button type="button" onClick={saveKeyLimit} disabled={saving} style={{ ...btnSmStyle, color: "var(--dash-accent)", borderColor: "var(--dash-accent)" }}>{saving ? "保存中" : "保存限制"}</button>
