...
本地未执行真实 AICards 同步和健康检查，因为当前本地 smoke 未提供生产 `AICARDS_API_KEY` 和管理员会话。代码层已实现：
...
| 产品经理 | 8.0 | 后台有同步、健康检查、审核、发布路径 | 老板后台仍偏技术语言 | 用“供货方、售价、毛利、发布”重写后台文案 |
| 技术工程师 | 8.2 | 路由隔离、响应脱敏、候选成本结算已补齐 | route_candidates 仍主要是审核记录，路由读取以 upstream_channels 为主 | 后续统一 route_candidates 为唯一候选表 |
...
| 路人测试 | 8.1 | 前台看不到 AICards/OpenRouter 等供应链名 | 管理后台仍需要懂“成本/售价/毛利” | 增加老板向导式发布流程 |
| UI 设计师 | 8.0 | 前台品牌统一，团队页可看成员/模型/Key 消耗 | 团队额度编辑仍偏表格 | 下一步做抽屉式额度编辑和移动端成员卡片 |
| QA | 8.1 | lint/build/smoke/泄露扫描通过 | 缺真实 AICards、真实扣费、邮箱验证码生产 E2E | 部署后用生产管理员账号执行完整 E2E |
...
- 真实 AICards 同步、健康检查和真实扣费 E2E 需要在生产环境验证。
