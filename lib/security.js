...
export function rateLimit(key, { limit, windowMs }) {
  if (process.env.NODE_ENV !== "production") {
    return { ok: true, remaining: Number(limit || 0), retryAfter: 0 };
  }
  const now = Date.now();
...
export function acquireConcurrency(key, max) {
  if (process.env.NODE_ENV !== "production") {
    concurrency.set(key, (concurrency.get(key) || 0) + 1);
    return true;
  }
...
export function securityLog(event, details = {}) {
  ...
  if (alertEmail && resendKey && shouldSendAlert(event)) {
    fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: normalizeFromHeader(process.env.EMAIL_FROM, DEFAULT_FROM),
        to: [alertEmail],
