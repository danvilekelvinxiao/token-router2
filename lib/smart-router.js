...
const TASK_MODEL_PREFERENCE = {
  coding: ["gpt-5.5", "gpt4o-mini", "qwen"],
  writing: ["gpt-5.5", "qwen", "gpt4o-mini"],
  analysis: ["gpt-5.5", "gpt4o-mini", "claude-haiku"],
  chat: ["gpt-5.5", "qwen", "gpt4o-mini"],
  math: ["gpt-5.5", "gpt4o-mini", "claude-haiku"],
  general: ["gpt-5.5", "qwen", "gpt4o-mini"],
};
...
const PAYMENT_REQUIRED_COOLDOWN_MS = Math.max(60_000, Number(process.env.ROUTE_402_COOLDOWN_MS || 15 * 60 * 1000));

function getPaymentCooldownState(candidate = {}, now = Date.now()) {
  const last402At = candidate.last402At ? new Date(candidate.last402At).getTime() : 0;
  if (!Number.isFinite(last402At) || last402At <= 0) {
    return { active: false, remainingMs: 0, until: "" };
  }
  const elapsed = now - last402At;
  const remainingMs = Math.max(0, PAYMENT_REQUIRED_COOLDOWN_MS - elapsed);
  return {
    active: remainingMs > 0,
    remainingMs,
    until: remainingMs > 0 ? new Date(now + remainingMs).toISOString() : "",
  };
}
...
async function loadRouteFailureSignals({ publicModelId = "", actualModelId = "" } = {}) {
  if (!hasDatabase()) {
    const attempts = globalThis.__TOKEN_ROUTER_CUSTOMERS__?.routeAttempts || [];
    const map = new Map();
    for (const row of attempts.filter((item) => !publicModelId || item.public_model_id === publicModelId || item.actual_model_id === actualModelId)) {
      const keys = [
        row.upstream_channel_id,
        row.upstream_channel,
      ...
      if (!entry.latestAt) {
        entry.latestRequestId = row.request_id || "";
        entry.latestStatusCode = Number(row.status_code || 0);
        entry.latestErrorCode = row.error_code || "";
        entry.latestErrorKind = row.error_kind || "";
        entry.latestErrorMessage = row.error_message || "";
        entry.latestAt = row.created_at || "";
        entry.latestPaymentRequired = row.payment_required === true;
        entry.latestUpstreamUrl = row.upstream_url || "";
        entry.latestApiKeyPreview = row.api_key_preview || "";
      }
      if (Number(row.status_code || 0) === 402 && !entry.last402At) {
        entry.last402RequestId = row.request_id || "";
        entry.last402At = row.created_at || "";
        entry.last402ErrorMessage = row.error_message || "";
      }
  ...
  try {
    const result = await query(
      `SELECT
         request_id,
         COALESCE(NULLIF(upstream_channel_id, ''), NULLIF(upstream_channel, ''), NULLIF(upstream_provider, ''), 'unknown') AS channel_key,
         upstream_channel_id,
         upstream_channel,
         upstream_provider,
         upstream_url,
...
function upstreamTierName(candidate = {}) {
  ...
  if (text.includes("openai official") || text.includes("official-openai") || text.includes("api.openai.com")) return "openrouter";
  if (text.includes("aicards") || text.includes("backup") || (text.includes("备用") && !text.includes("openrouter"))) return "backup";
  if (text.includes("uniapi")) return "backup";
  return "backup";
}
...
function scoreCandidates(candidates = [], healthMap = new Map(), { strategy = "balanced", usageEstimate = {} } = {}) {
  ...
  return candidates.map((candidate) => {
    ...
    const paymentCooldown = getPaymentCooldownState(candidate, now);
    ...
    const baseAvailabilityScore = clampScore(observedSuccessRate * 100, health.ok ? 90 : 30);
    const availabilityScore = paymentCooldown.active ? Math.min(baseAvailabilityScore, 12) : baseAvailabilityScore;
    ...
    ) - Math.max(0, candidate.priority - 1) * 0.5 - (paymentCooldown.active ? 35 : 0);
    ...
    return {
      ...
      health,
      paymentCooldownActive: paymentCooldown.active,
      paymentCooldownRemainingMs: paymentCooldown.remainingMs,
      paymentCooldownUntil: paymentCooldown.until,
    };
  }).sort((a, b) => {
...
export async function selectUpstream({
  ...
  if (String(effectivePublicModelId).trim().toLowerCase() === "gpt-5.5" || String(effectiveActualModelId).trim().toLowerCase() === "gpt-5.5") {
    const pinnedCandidates = await getRouteCandidates({
      publicModelId: effectivePublicModelId,
      actualModelId: effectiveActualModelId,
      modelProduct,
    });
    const configuredSub2apiBase = normalizeBaseUrl(process.env.SUB2API_BASE_URL || process.env.SUB2API_INTERNAL_URL || "");
    const sub2apiCandidates = pinnedCandidates.filter((candidate) => {
      const text = `${candidate.id} ${candidate.name} ${candidate.channelName} ${candidate.providerName} ${candidate.upstreamName}`.toLowerCase();
      return text.includes("sub2api");
    });
    const sub2apiCandidate = sub2apiCandidates.find((candidate) => {
      const candidateBase = normalizeBaseUrl(candidate.baseUrl || candidate.raw?.base_url || candidate.raw?.baseUrl || "");
      const candidateUpstream = normalizeBaseUrl(candidate.upstreamUrl || candidate.raw?.upstream_url || "");
      return (
        (configuredSub2apiBase && candidateBase === configuredSub2apiBase) ||
        (configuredSub2apiBase && candidateUpstream.startsWith(configuredSub2apiBase)) ||
        candidateBase.includes("127.0.0.1:8080") ||
        candidateBase.includes("localhost:8080")
      );
    }) || sub2apiCandidates.find((candidate) => {
      const candidateBase = normalizeBaseUrl(candidate.baseUrl || candidate.raw?.base_url || candidate.raw?.baseUrl || "");
      return candidateBase && !candidateBase.includes("aicards.shop");
    }) || sub2apiCandidates[0] || pinnedCandidates[0] || null;
    if (sub2apiCandidate) {
      const decision = {
        upstream: sub2apiCandidate,
        channel: sub2apiCandidate,
        strategy: "sub2api_pinned",
        reason: configuredSub2apiBase
          ? `gpt-5.5 固定走 sub2api（${configuredSub2apiBase}）`
          : "gpt-5.5 固定走 sub2api",
        candidates: pinnedCandidates,
        fallbackChain: dedupeUpstreamCandidates(pinnedCandidates),
      };
      cache.set("routeDecision", `${effectivePublicModelId || "any"}:${hashCacheKey("sub2api-pinned")}`, decision, CACHE_TTLS.routeDecision);
      return decision;
    }
  }
  ...
  const failureSignals = await loadRouteFailureSignals({ publicModelId: effectivePublicModelId, actualModelId: effectiveActualModelId });
  const withMetrics = candidates.map((candidate) => {
    const metric = metrics.get(String(candidate.upstreamName || candidate.channelName || "").toLowerCase())
      ...
      || {};
    const failure = failureSignals.get(String(candidate.id || candidate.upstreamName || candidate.channelName || candidate.providerName || "").toLowerCase())
      || failureSignals.get(String(candidate.channelName || "").toLowerCase())
      || failureSignals.get(String(candidate.providerName || "").toLowerCase())
      || {};
    return {
      ...candidate,
      ...
      latestRequestId: failure.latestRequestId || "",
      latestStatusCode: failure.latestStatusCode || 0,
      latestErrorCode: failure.latestErrorCode || "",
      latestErrorKind: failure.latestErrorKind || "",
      latestErrorMessage: failure.latestErrorMessage || "",
      latestAt: failure.latestAt || "",
      latestPaymentRequired: Boolean(failure.latestPaymentRequired),
      latestUpstreamUrl: failure.latestUpstreamUrl || candidate.upstreamUrl || "",
      latestApiKeyPreview: failure.latestApiKeyPreview || "",
      last402RequestId: failure.last402RequestId || "",
      last402At: failure.last402At || "",
      last402ErrorMessage: failure.last402ErrorMessage || "",
    };
  });
      ...
      try {
        ...
        return {
          ...
          candidates: (scoredDecision.candidates?.length ? scoredDecision.candidates : candidates).map((candidate) => ({
            ...
            latestRequestId: candidate.latestRequestId || "",
            latestStatusCode: candidate.latestStatusCode || 0,
            latestErrorCode: candidate.latestErrorCode || "",
            latestErrorKind: candidate.latestErrorKind || "",
            latestErrorMessage: candidate.latestErrorMessage || "",
            latestAt: candidate.latestAt || "",
            latestPaymentRequired: Boolean(candidate.latestPaymentRequired),
            latestUpstreamUrl: candidate.latestUpstreamUrl || "",
            latestApiKeyPreview: candidate.latestApiKeyPreview || "",
            last402RequestId: candidate.last402RequestId || "",
            last402At: candidate.last402At || "",
            last402ErrorMessage: candidate.last402ErrorMessage || "",
            paymentCooldownActive: Boolean(candidate.paymentCooldownActive),
            paymentCooldownRemainingMs: Number(candidate.paymentCooldownRemainingMs || 0),
            paymentCooldownUntil: candidate.paymentCooldownUntil || "",
          })),
        };
      } catch (error) {
        return {
