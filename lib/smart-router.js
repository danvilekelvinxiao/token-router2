...
import { getProviderRouteCandidates } from "./provider-store";
...
function routingPreferenceTier(candidate = {}) {
  const text = [
    candidate.id,
    candidate.name,
    candidate.label,
    ...
    candidate.raw?.provider?.type,
    candidate.raw?.provider?.name,
  ].filter(Boolean).join(" ").toLowerCase();

  const isOpenrouter = text.includes("openrouter");
  const isAicardsBackup = text.includes("aicards") || text.includes("aicards.shop");
  const isSub2api = text.includes("sub2api") || text.includes("flowapi 主线路") || text.includes("flowapi 备线路") || text.includes("flowapi claude 线路");
  const isRelay = text.includes("new-api")
    || text.includes("newapi")
    || text.includes("uniapi")
    || text.includes("aheapi")
    || text.includes("bridge")
    || text.includes("azure");

  if (isOpenrouter) return 3;
  if (isSub2api && !isAicardsBackup) return 0;
  if (isSub2api || isRelay || isAicardsBackup) return 1;
  return 2;
}

function sortCandidatesForStableRouting(candidates = []) {
  return [...candidates].sort((left, right) => {
    const tierDiff = routingPreferenceTier(left) - routingPreferenceTier(right);
...
function isGpt55Route(modelId = "", publicModelId = "", modelProduct = null) {
  const text = [
    modelId,
    publicModelId,
    modelProduct?.id,
  ...
  return text.includes("gpt-5.5") || text.includes("gpt55") || text.includes("flowapi-gpt55") || text.includes("chatgpt5.5") || text.includes("chatgpt55");
}
...
function upstreamForChannel(channel = {}, upstreams = []) {
  ...
  const providerKey = String(channel.providerKey || channel.provider_key || channel.raw?.provider_key || "").toLowerCase();
  const name = String(channel.name || channel.channelName || channel.channel_name || "").toLowerCase();

  // If a channel declares a concrete baseUrl, never match it to another upstream
  // just because providerKey/name text is similar. This prevents an aicards.shop
  // backup channel labelled "sub2api" from stealing the A primary route.
  if (baseUrl) {
    return upstreams.find((upstream) => normalizeBaseUrl(upstream.baseUrl) === baseUrl) || null;
  }

  return upstreams.find((upstream) => {
    const upstreamName = String(upstream.name || "").toLowerCase();
    return (
      (providerKey && upstreamName && providerKey === upstreamName) ||
...
function normalizeChannelCandidate(channel = {}, upstream = null, modelProduct = null, endpoint = "chat") {
  ...
  const chatCompletionsUrl = upstream?.chatCompletionsUrl || upstream?.upstreamUrl || (baseUrl ? `${baseUrl}${apiPath.startsWith("/") ? apiPath : `/${apiPath}`}` : "");
  const responsesUrl = upstream?.responsesUrl || (baseUrl ? `${baseUrl}/v1/responses` : "");
  const supportsResponsesApi = upstream?.supportsResponsesApi ?? false;
  const supportsChatCompletionsApi = upstream?.supportsChatCompletionsApi ?? true;
  const useResponsesEndpoint = endpoint === "responses" && supportsResponsesApi && responsesUrl;
  const upstreamUrl = useResponsesEndpoint ? responsesUrl : chatCompletionsUrl;
  const providerKey = String(channel.provider_key || channel.providerKey || upstream?.name || "").trim();
  return {
    ...
    profitProtectionHits: Number(channel.profit_protection_hits ?? channel.profitProtectionHits ?? 0),
    raw: channel,
  };
}
...
function hydrateCandidateSecrets(candidate = {}, upstreams = []) {
  ...
  return {
    ...
    chatCompletionsUrl: candidate.chatCompletionsUrl || upstream.chatCompletionsUrl || upstream.upstreamUrl,
    responsesUrl: candidate.responsesUrl || upstream.responsesUrl,
    ...
    supportsResponsesApi: upstream.supportsResponsesApi ?? candidate.supportsResponsesApi,
    supportsChatCompletionsApi: upstream.supportsChatCompletionsApi ?? candidate.supportsChatCompletionsApi,
  };
}
...
function candidateRequiresConfiguredCost(candidate = {}) {
  const haystack = [
    candidate.id,
    candidate.name,
    candidate.providerName,
  ...
  return Boolean(candidate.raw?.id && candidate.raw?.base_url)
    || haystack.includes("aicards")
    || haystack.includes("backup")
    || haystack.includes("备用");
}
...
function scoreCandidates(candidates = [], healthMap = new Map(), { strategy = "balanced", usageEstimate = {}, userChargeEstimate = 0 } = {}) {
  ...
  return candidates.map((candidate) => {
    ...
    const missingRequiredCost = candidateRequiresConfiguredCost(candidate)
      && Number(candidate.inputCostPerMillion || 0) <= 0
      && Number(candidate.outputCostPerMillion || 0) <= 0;
    ...
    const profitProtected = missingRequiredCost || (userChargeEstimate > 0 && upstreamCost >= userChargeEstimate * 0.85);
    ...
    if (a.profitProtected !== b.profitProtected) return a.profitProtected ? 1 : -1;
    return b.score - a.score || a.priority - b.priority;
  });
}

async function recordProfitProtectionHits(candidates = []) {
  const ids = candidates.map((candidate) => candidate.id).filter(Boolean);
  if (!ids.length || !hasDatabase()) return;
  try {
    await query(
      `UPDATE upstream_channels
       SET profit_protection_hits = profit_protection_hits + 1,
           updated_at = NOW()
       WHERE id = ANY($1::text[])`,
      [ids]
    );
  } catch {
    // Profit guard statistics must never block user requests.
  }
}
...
function sanitizeRoutePreviewCandidate(candidate = {}) {
  ...
  return {
    ...
    profitProtected: candidate.profitProtected,
...
export async function getRouteCandidates({
  ...
  const cacheKey = `${publicModelId || "any"}:${actualModelId || "any"}:${includeDisabled ? "all" : "enabled"}:${endpoint}`;
  ...
  const dbCandidates = dbChannels.map((channel) => normalizeChannelCandidate(channel, upstreamForChannel(channel, upstreams), modelProduct, endpoint));
  const providerCandidates = await getProviderRouteCandidates(publicModelId || actualModelId, { endpoint }).catch(() => []);
  ...
  const adminCandidates = adminChannels
    ...
    .map((channel) => normalizeChannelCandidate(channel, upstreamForChannel(channel, upstreams), modelProduct, endpoint));
  ...
  const upstreamCandidates = upstreams
    ...
    .map((upstream, index) => normalizeChannelCandidate({
      ...
      priority: index + 10,
    }, upstream, modelProduct, endpoint));

  const deduped = new Map();
  [...providerCandidates, ...dbCandidates, ...adminCandidates, ...upstreamCandidates].forEach((candidate) => {
    if (!candidate.upstreamUrl) return;
    if (!includeDisabled && !candidate.isEnabled) return;
    const key = `${candidate.id}:${candidate.upstreamUrl}`;
...
export async function selectUpstream({
  ...
  const cache = getCacheManager();
  const cachedDecision = cache.get("routeDecision", decisionCacheKey);
  if (cachedDecision) {
  ...
  if (preferredChannelId) {
    ...
    if (preferred) {
      const decision = {
        ...
        candidates,
        fallbackChain: [preferred, ...candidates.filter((candidate) => candidate.id !== preferred.id)],
      };
      cache.set("routeDecision", decisionCacheKey, decision, CACHE_TTLS.routeDecision);
      return decision;
    }
  }

  if (!candidates.length) {
    const defaultUpstreams = sortCandidatesForStableRouting(
      upstreams.filter((upstream) => upstream.includeAsDefaultCandidate !== false)
    );

    // GPT-5.5 must always route to sub2api — enforce even when no DB/admin candidates are present
    if (isGpt55Route(modelId, effectivePublicModelId, modelProduct)) {
      const sub2apiDefault = defaultUpstreams.find((upstream) => {
        const text = [upstream.name, upstream.label, upstream.baseUrl].filter(Boolean).join(" ").toLowerCase();
        return text.includes("sub2api") && !text.includes("aicards");
      });
      if (sub2apiDefault) {
        const decision = {
          upstream: sub2apiDefault,
          channel: sub2apiDefault,
          strategy: activeStrategy,
          reason: "gpt-5.5 固定优先 sub2api（无候选配置，使用默认上游）",
          candidates: [],
          fallbackChain: [sub2apiDefault, ...defaultUpstreams.filter((u) => u !== sub2apiDefault)],
        };
        cache.set("routeDecision", decisionCacheKey, decision, CACHE_TTLS.routeDecision);
        return decision;
      }
    ...
    return {
      ...
      strategy: activeStrategy,
      reason: "未找到已启用候选渠道，使用默认上游（sub2api 优先，OpenRouter 最后）",
      candidates: [],
  ...
  const scored = sortCandidatesForStableRouting(scoreCandidates(withMetrics, healthMap, {
    strategy: activeStrategy,
    usageEstimate,
    userChargeEstimate,
  }));
  const available = scored.filter((candidate) => !candidate.profitProtected);

  if (isGpt55Route(modelId, effectivePublicModelId, modelProduct)) {
    const isAicardsBackup = (candidate = {}) => {
      const text = [candidate.baseUrl, candidate.upstreamUrl, candidate.id, candidate.name, candidate.providerName, candidate.channelName].filter(Boolean).join(" ").toLowerCase();
      return text.includes("aicards.shop") || text.includes("aicards");
    };
    const isPrimarySub2api = (candidate = {}) => {
      const text = [candidate.id, candidate.name, candidate.providerName, candidate.providerKey, candidate.channelName, candidate.groupName].filter(Boolean).join(" ").toLowerCase();
      return !isAicardsBackup(candidate) && (text.includes("sub2api") || text.includes("flowapi-gpt55"));
    };
    const isBackupSub2api = (candidate = {}) => {
      const text = [candidate.id, candidate.name, candidate.providerName, candidate.providerKey, candidate.channelName, candidate.groupName].filter(Boolean).join(" ").toLowerCase();
      return text.includes("sub2api") || isAicardsBackup(candidate) || text.includes("flowapi-gpt55");
    };
    const isOpenrouterCandidate = (candidate = {}) => {
      const text = [candidate.baseUrl, candidate.upstreamUrl, candidate.id, candidate.name, candidate.providerName, candidate.channelName].filter(Boolean).join(" ").toLowerCase();
      return text.includes("openrouter");
    };
    const sub2apiCandidate = available.find(isPrimarySub2api) || available.find(isBackupSub2api);
    if (sub2apiCandidate) {
      const orderedBackups = available
        .filter((candidate) => candidate.id !== sub2apiCandidate.id)
        .sort((left, right) => {
          const leftOpenrouter = isOpenrouterCandidate(left);
          const rightOpenrouter = isOpenrouterCandidate(right);
          if (leftOpenrouter !== rightOpenrouter) return leftOpenrouter ? 1 : -1;
          return Number(left.priority || 999) - Number(right.priority || 999);
        });
      const fallbackChain = [sub2apiCandidate, ...orderedBackups];
      const decision = {
        upstream: sub2apiCandidate,
        channel: sub2apiCandidate,
        strategy: activeStrategy,
        reason: `gpt-5.5 固定优先 sub2api：综合评分 ${sub2apiCandidate.score}`,
        candidates: scored,
        fallbackChain,
      };
      cache.set("routeDecision", decisionCacheKey, decision, CACHE_TTLS.routeDecision);
  ...
  if (!available.length) {
    await recordProfitProtectionHits(scored);
    const decision = {
      error: "profit_protected",
      upstream: null,
      strategy: activeStrategy,
      reason: "该模型当前上游成本过高，暂时维护中，请稍后再试。",
      candidates: scored,
      fallbackChain: [],
    };
    cache.set("routeDecision", decisionCacheKey, decision, CACHE_TTLS.routeDecision);
  ...
  const decision = {
    upstream: available[0],
    channel: available[0],
    strategy: activeStrategy,
    reason: `智能选择 ${available[0].channelName}：综合评分 ${available[0].score}`,
    candidates: scored,
      ...
      try {
        ...
        return {
          ...
          candidates: (scoredDecision.candidates?.length ? scoredDecision.candidates : candidates).map((candidate) => ({
            ...
            avgFirstTokenMs: candidate.avgFirstTokenMs,
            p95LatencyMs: candidate.p95LatencyMs,
            profitProtectionHits: candidate.profitProtectionHits,
