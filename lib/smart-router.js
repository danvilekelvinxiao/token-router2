...
import { compareRouteCandidates, filterRouteCandidatesByPolicy, getRoutePolicyConfig, getRouteTierIndex, routeTierForCandidate } from "./route-policy";
import { isUpstreamCoolingDown, reportUpstreamFailure, clearUpstreamFailure } from "./upstream-failure-state";
...
export async function getUpstreamHealth(upstream, { ttlMs = CACHE_TTLS.upstreamHealth } = {}) {
  ...
  try {
    ...
    const method = String(upstream.healthMethod || "GET").toUpperCase();
    const body = method === "POST" && upstream.healthBody ? JSON.stringify(upstream.healthBody) : undefined;
...
function modelMatchesChannel(channel = {}, modelId = "") {
  ...
  const family = String(channel.family || channel.modelFamily || "").toLowerCase();
  if (family === "claude") return normalized.includes("claude");
  if (family === "non-claude" || family === "default") return !normalized.includes("claude");
...
function upstreamForChannel(channel = {}, upstreams = []) {
  ...
  const providerKey = String(channel.provider_key || channel.providerKey || "").toLowerCase();
  return upstreams.find((upstream) => {
    ...
    const upstreamLabel = String(upstream.label || "").toLowerCase();
    return (
      (providerKey && upstreamName === providerKey) ||
      (providerKey && upstreamLabel === providerKey) ||
...
function normalizeChannelCandidate(channel = {}, upstream = null, modelProduct = null) {
  ...
  return {
    ...
    healthMethod: upstream?.healthMethod || channel.healthMethod || channel.health_method || "GET",
    healthBody: upstream?.healthBody || channel.healthBody || channel.health_body || null,
    family: upstream?.family || channel.family || channel.modelFamily || "",
...
function scoreCandidates(candidates = [], healthMap = new Map(), { strategy = "balanced", usageEstimate = {}, userChargeEstimate = 0, routePolicy = null } = {}) {
  ...
  return candidates.map((candidate) => {
    ...
    const coolingDown = isUpstreamCoolingDown(candidate);
    ...
    const routeTierIndex = getRouteTierIndex(candidate, routePolicy || undefined);
    const score = (
    ...
    return {
      ...candidate,
      routeTier: routeTierForCandidate(candidate),
      routeTierIndex,
    ...
    if (a.routeTierIndex !== b.routeTierIndex) return a.routeTierIndex - b.routeTierIndex;
...
export async function getRouteCandidates({
  ...
  const policyKey = hashCacheKey(JSON.stringify(routePolicy || {}));
  const cacheKey = `${publicModelId || "any"}:${actualModelId || "any"}:${includeDisabled ? "all" : "enabled"}:${policyKey}`;
  ...
  const candidates = filterRouteCandidatesByPolicy(Array.from(deduped.values()), routePolicy || undefined);
  candidates.sort((a, b) => compareRouteCandidates(a, b, routePolicy || undefined));
...
export async function selectUpstream({
  ...
  if (cachedDecision) {
    ...
    if (selected && selected.health?.ok && !selected.coolingDown) {
      return {
        ...cachedDecision,
        upstream: selected,
        channel: selected,
  ...
  const decision = {
    ...
    channel: available[0],
    strategy: activeStrategy,
    reason: `智能选择 ${available[0].channelName}：综合评分 ${available[0].score}${available[0].health?.ok === false ? "（健康检查未通过，仍按优先级尝试）" : ""}`,
    candidates: scored,
