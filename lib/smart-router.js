...
function upstreamTierName(candidate = {}) {
  ...
  if (text.includes("aicards") || text.includes("backup") || text.includes("备用")) return "backup";
...
function orderCandidatesByRoutePolicy(candidates = [], fallbackOrder = []) {
  const order = Array.isArray(fallbackOrder) && fallbackOrder.length ? fallbackOrder : ["sub2api", "newApi", "backup", "uniapi", "openrouter"];
