...
async function loadDatabaseChannels({ publicModelId = "", actualModelId = "" } = {}) {
...
function scoreCandidates(candidates = [], healthMap = new Map(), { strategy = "balanced", usageEstimate = {}, userChargeEstimate = 0 } = {}) {
  ...
  return candidates.map((candidate) => {
    ...
    const profitProtected = userChargeEstimate > 0 && upstreamCost >= userChargeEstimate * 0.85;
...
export async function getRouteCandidates({
  ...
  const dbChannels = await loadDatabaseChannels({ publicModelId, actualModelId });
  ...
  const upstreamCandidates = upstreams.map((upstream, index) => normalizeChannelCandidate({
    id: upstream.name,
