...
function isAllowedHost(hostname = "") {
  ...
  if (process.env.FLOWAPI_ALLOW_PRIVATE_UPSTREAMS === "true") return true;
...
function isDevPrivateHost(hostname = "") {
  const host = String(hostname || "").toLowerCase();
  if (!host) return false;
...
export async function assertSafeUpstreamUrl(input = "", { resolveDns = true } = {}) {
  ...
  if (!allowed) {
    if (process.env.FLOWAPI_ALLOW_PRIVATE_UPSTREAMS === "true" && isDevPrivateHost(hostname)) {
