...
export function makeRechargeDescription(amount) {
  return `FlowAPI 钱包充值 ¥${Number(amount).toFixed(2)}`;
}

export function getRechargePaymentAmount(order = {}) {
  return Number(order.paymentAmountRmb ?? order.payment_amount_rmb ?? order.amount ?? 0);
}
