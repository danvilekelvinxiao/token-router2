...
function buildPlanProgress({ wallet = {}, plan = null, calls = [], now = new Date() } = {}) {
  ...
  const parsedTotalTokens = parseQuotaTokens(plan.quotaText);
  const totalTokens = Number.isFinite(Number(plan.totalTokens))
    ? Number(plan.totalTokens)
    : parsedTotalTokens;
  const planCalls = calls.filter((call) => isWithinRange(call?.createdAt, plan.startedAt, plan.expiresAt));
  const usedTokens = Number.isFinite(Number(plan.usedTokens))
    ? Number(plan.usedTokens)
    : (totalTokens
      ? planCalls.reduce((sum, call) => sum + toNumber(call?.tokens, toNumber(call?.promptTokens) + toNumber(call?.completionTokens)), 0)
      : null);
  const remainingTokens = Number.isFinite(Number(plan.remainingTokens))
    ? Number(plan.remainingTokens)
    : (totalTokens && usedTokens !== null ? Math.max(0, totalTokens - usedTokens) : null);
  const totalCny = Number.isFinite(Number(plan.totalCny))
    ? Number(plan.totalCny)
    : toNumber(safeWallet.totalQuotaCny);
  const usedCny = Number.isFinite(Number(plan.usedCny))
    ? Number(plan.usedCny)
    : toNumber(safeWallet.usedQuotaCny);
