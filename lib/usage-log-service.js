import { getCustomer, listActivityLogs, listCustomerCalls, listCustomers, listRechargeOrders } from "@/lib/customer-store";
...
export async function buildUsageLogs({ customerId, filters = {}, limit = 200 } = {}) {
  ...
  const calls = await listCustomerCalls(customerId, { limit: Number(limit) > 0 ? Number(limit) : null });

  const apiRows = calls.map((call) => {
