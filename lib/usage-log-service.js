...
import { formatDateTime } from "@/lib/export/server-excel";
...
function statusLabel(status) {
  ...
  if (["success", "paid", "approved", "completed", "success_completed"].includes(text)) return "成功";
  if (["stream_incomplete", "client_aborted", "downstream_failed", "upstream_completed_downstream_failed", "usage_missing"].includes(text)) return "响应未完整返回";
  if (["billing_refunded", "refunded", "refund_credit"].includes(text)) return "已自动退回";
  if (["billing_pending_reconciliation", "pending"].includes(text)) return "待处理";
  if (text === "partial_success") return "部分成功";
  if (["failed", "upstream_error", "stream_failed", "not_billed"].includes(text)) return "失败";
...
function applyFilters(items = [], filters = {}) {
  ...
  return items
    ...
    .filter((item) => !requestId || String(item.requestId || item.id || "").toLowerCase().includes(requestId) || String(item.callId || "").toLowerCase().includes(requestId))
...
export async function buildUsageLogs({ customerId, filters = {}, limit = 200 } = {}) {
  ...
  const apiRows = (customer.calls || []).map((call) => {
    ...
    return {
      ...
      type: ["失败", "响应未完整返回"].includes(statusLabel(call.billingStatus || call.deliveryStatus || call.status)) ? "failed" : "consume",
      apiKeyId: call.apiKeyId || "",
      apiKeyLabel: apiKey.label || "未命名密钥",
      apiKeyMasked: apiKey.label || "未命名密钥",
      ...
      status: statusLabel(call.billingStatus || call.deliveryStatus || call.status),
      billingStatus: call.billingStatus || "",
      deliveryStatus: call.deliveryStatus || "",
      normalizedStatus: ["失败", "响应未完整返回"].includes(statusLabel(call.billingStatus || call.deliveryStatus || call.status)) ? "failed" : "success",
  ...
  const imageRows = (imageResult.items || []).map((item) => ({
    ...
    apiKeyLabel: getKey(apiKeys, item.apiKeyId)?.label || "未命名密钥",
    apiKeyMasked: getKey(apiKeys, item.apiKeyId)?.label || "未命名密钥",
    group: "图片生成",
  ...
  return {
    ...
    apiKeys: apiKeys.map((key) => ({
      id: key.id,
      label: key.label || "未命名密钥",
      masked: key.label || "未命名密钥",
...
export async function buildAdminUsageLogs({ filters = {}, limit = 5000 } = {}) {
  ...
  const apiKeys = customers.flatMap((customer) => (customer.apiKeys || []).map((key) => ({
    id: key.id,
    label: `${customer.name || customer.email || customer.id} · ${key.label || "未命名密钥"}`,
    masked: key.label || "未命名密钥",
    modelGroup: key.modelGroup || "",
  })));
