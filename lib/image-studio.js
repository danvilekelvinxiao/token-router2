import { checkApiKeyLimitById, getCustomer, findCustomerByToken, recordApiKeyLimitUsage } from "@/lib/customer-store";
...
const DEFAULT_IMAGE_MODELS = [
  {
    ...
    display_name: "GPT-5.4 Image 2 Pro",
  ...
  if (!imageStudioInitPromise) {
    imageStudioInitPromise = query(`
      CREATE TABLE IF NOT EXISTS image_models (
        ...
        image_billing_mode TEXT NOT NULL DEFAULT 'legacy_rmb',
      ...
      ALTER TABLE image_models ADD COLUMN IF NOT EXISTS image_billing_mode TEXT NOT NULL DEFAULT 'legacy_rmb';
      ALTER TABLE image_models ADD COLUMN IF NOT EXISTS image_cost_per_image_cny NUMERIC(14, 6) NOT NULL DEFAULT 0;
      ALTER TABLE image_models ADD COLUMN IF NOT EXISTS image_fixed_profit_per_image_cny NUMERIC(14, 6) NOT NULL DEFAULT 0;
      ALTER TABLE image_models ADD COLUMN IF NOT EXISTS image_sell_price_per_image_cny NUMERIC(14, 6) NOT NULL DEFAULT 0;
      ALTER TABLE image_generation_logs ADD COLUMN IF NOT EXISTS upstream_cost_cny NUMERIC(14, 6) NOT NULL DEFAULT 0;
      ALTER TABLE image_generation_logs ADD COLUMN IF NOT EXISTS sell_price_cny NUMERIC(14, 6) NOT NULL DEFAULT 0;
      ALTER TABLE image_generation_logs ADD COLUMN IF NOT EXISTS profit_cny NUMERIC(14, 6) NOT NULL DEFAULT 0;
      ALTER TABLE image_generation_logs ADD COLUMN IF NOT EXISTS profit_margin NUMERIC(8, 4) NOT NULL DEFAULT 0;
      ALTER TABLE image_generation_logs ADD COLUMN IF NOT EXISTS billing_mode TEXT DEFAULT '';
      ALTER TABLE image_generation_logs ADD COLUMN IF NOT EXISTS image_count INTEGER NOT NULL DEFAULT 0;
      ALTER TABLE image_generation_logs ADD COLUMN IF NOT EXISTS cost_per_image_cny NUMERIC(14, 6) NOT NULL DEFAULT 0;
      ALTER TABLE image_generation_logs ADD COLUMN IF NOT EXISTS sell_price_per_image_cny NUMERIC(14, 6) NOT NULL DEFAULT 0;
...
function mapImageModelRow(row = {}) {
  ...
  const upstreamModel = row.upstream_model || row.upstreamModel;
  const identity = `${provider} ${displayName} ${upstreamModel || ""} ${row.id || ""}`.toLowerCase();
  const tags = Array.isArray(row.tags) ? row.tags : typeof row.tags === "string" ? JSON.parse(row.tags || "[]") : [];
  const logo = identity.includes("openai") || identity.includes("gpt")
    ? "openai"
    : identity.includes("google") || identity.includes("gemini") || identity.includes("imagen")
      ? "google"
      : identity.includes("x-ai") || identity.includes("xai") || identity.includes("grok")
        ? "xai"
      : identity.includes("anthropic") || identity.includes("claude")
        ? "anthropic"
        : identity.includes("deepseek")
          ? "deepseek"
          : identity.includes("qwen") || identity.includes("alibaba")
            ? "alibaba"
            : identity.includes("flux") || identity.includes("black-forest")
              ? "flux"
              : identity.includes("stability") || identity.includes("stable-diffusion")
                ? "stability"
                : identity.includes("midjourney")
                  ? "midjourney"
                  : identity.includes("kling") || identity.includes("kuaishou")
                    ? "kling"
                    : identity.includes("seedream") || identity.includes("bytedance") || identity.includes("doubao")
                      ? "bytedance"
                      : "image";
  return {
    id: row.id,
    ...
    logoCode: logo,
    capabilities: {
      textToImage: row.supports_text_to_image !== false,
      imageToImage: Boolean(row.supports_image_to_image),
      multiImage: Boolean(row.supports_image_to_image && row.supports_batch),
      videoInput: false,
      fileInput: false,
    },
    tier: identity.includes("mini") ? "mini" : identity.includes("pro") || identity.includes("ultra") ? "pro" : "standard",
    family: String(upstreamModel || row.id || "").split("/").pop()?.split("-").slice(0, 3).join("-") || "image",
    version: String(displayName || upstreamModel || "").match(/\b\d+(?:\.\d+)?\b/)?.[0] || "",
    ...
    imageBillingMode: row.image_billing_mode || row.imageBillingMode || "legacy_rmb",
    imageCostPerImageCny: Number(row.image_cost_per_image_cny || row.imageCostPerImageCny || 0),
    imageFixedProfitPerImageCny: Number(row.image_fixed_profit_per_image_cny || row.imageFixedProfitPerImageCny || 0),
    imageSellPricePerImageCny: Number(row.image_sell_price_per_image_cny || row.imageSellPricePerImageCny || 0),
...
function mapImageLogRow(row = {}) {
  ...
  return {
    ...
    sellPriceCny: Number(row.sell_price_cny || row.sellPriceCny || row.money_cost || row.moneyCost || 0),
    ...
    imageCount: Number(row.image_count || row.imageCount || row.output_image_count || row.outputImageCount || 0),
    costPerImageCny: Number(row.cost_per_image_cny || row.costPerImageCny || 0),
    sellPricePerImageCny: Number(row.sell_price_per_image_cny || row.sellPricePerImageCny || 0),
...
function sanitizeImageLogForViewer(log = {}, canViewInternal = false) {
  if (canViewInternal) return log;
  return {
    ...log,
    upstreamProvider: "FlowAPI",
    upstreamModel: log.modelDisplayName || "FlowAPI 图片模型",
    errorSource: log.errorCode ? "FlowAPI" : "",
  };
}

function imagePricingError(message, code = "IMAGE_MODEL_PROFIT_GUARD") {
  ...
  if (!hasDatabase()) {
    ...
    if (delta < 0) {
      ...
      memoryCustomer.totalSpend = roundMoney(Math.max(0, Number(memoryCustomer.totalSpend || 0) - Math.abs(delta)));
	      ...
	      memoryCustomer.balance = roundMoney(Math.max(0, Number(memoryCustomer.balance || 0) - delta));
...
export async function listImageModels({ includeDisabled = false } = {}) {
  const readMemoryModels = () => {
    syncMemoryImageModels();
    return imageStudioStore.imageModels
      .filter((item) => includeDisabled || item.enabled)
      .sort((a, b) => Number(a.sort_order || 999) - Number(b.sort_order || 999))
      .map(mapImageModelRow);
  };

  try {
    await ensureImageStudioSchema();
    if (!hasDatabase()) {
      return readMemoryModels();
    }
    const result = await query(
...
export function mapPublicImageModel(model = {}) {
  const rawTags = Array.isArray(model.labelTags)
    ? model.labelTags
  ...
  const blockedTagPattern = /openrouter|openai|google|grok|x-ai|xai|anthropic|claude|bytedance|doubao|seedream|flux|black[-\s]?forest|stability|stable[-\s]?diffusion|midjourney|kling|kuaishou|qwen|alibaba/i;
  const tags = rawTags
    .map((tag) => tagMap[String(tag || "").trim()] || String(tag || "").trim())
    .filter(Boolean)
    .filter((tag) => !blockedTagPattern.test(tag))
    .filter((tag, index, list) => list.indexOf(tag) === index);
  return {
    id: model.id,
    modelId: model.id,
    publicModelId: model.id,
    displayName: model.displayName || model.id || "FlowAPI 图片模型",
    provider: "FlowAPI",
    providerName: "FlowAPI",
    logo: "image",
    logoCode: "image",
    capabilities: model.capabilities || {
      textToImage: model.supportsTextToImage !== false,
      imageToImage: Boolean(model.supportsImageToImage),
      multiImage: Boolean(model.supportsMultiImage || model.supportsBatch),
      videoInput: false,
      fileInput: false,
    },
    tier: model.tier === "mini" || model.tier === "pro" ? model.tier : "standard",
    family: "image",
    version: "",
    enabled: model.enabled !== false,
    recommended: Boolean(model.recommended),
    supportsTextToImage: model.supportsTextToImage !== false,
    supportsImageToImage: Boolean(model.supportsImageToImage),
    supportsBatch: Boolean(model.supportsBatch),
    supportsMultiImage: Boolean(model.supportsMultiImage || model.supportsBatch),
    supportsVideoInput: Boolean(model.supportsVideoInput),
    supportsFileInput: Boolean(model.supportsFileInput),
    speedScore: Number(model.speedScore || 0),
    stabilityScore: Number(model.stabilityScore || 0),
    qualityScore: Number(model.qualityScore || 0),
    unitPriceTokenTextToImage: Number(model.unitPriceTokenTextToImage || 0),
    unitPriceTokenImageToImage: Number(model.unitPriceTokenImageToImage || 0),
    unitPriceRmbTextToImage: Number(model.unitPriceRmbTextToImage || 0),
    unitPriceRmbImageToImage: Number(model.unitPriceRmbImageToImage || 0),
    imageBillingMode: model.imageBillingMode || "legacy_rmb",
    imageSellPricePerImageCny: Number(model.imageSellPricePerImageCny || model.unitPriceRmbTextToImage || 0),
    tags,
    labelTags: tags,
    sceneDescription: model.sceneDescription || "",
    sortOrder: Number(model.sortOrder || 999),
  };
}
...
export async function updateImageModelConfig(modelId, updates = {}) {
  ...
  const fields = {
    ...
    image_billing_mode: updates.imageBillingMode,
    image_cost_per_image_cny: updates.imageCostPerImageCny,
    image_fixed_profit_per_image_cny: updates.imageFixedProfitPerImageCny,
    image_sell_price_per_image_cny: updates.imageSellPricePerImageCny,
  ...
  if (!hasDatabase()) {
    ...
    if (!target) {
      ...
      Object.entries(fields).forEach(([key, value]) => {
        if (value !== undefined) created[key] = key === "tags" ? JSON.parse(value) : value;
      });
      imageStudioStore.imageModels.push(created);
      return mapImageModelRow(created);
    }
    Object.entries(fields).forEach(([key, value]) => {
  ...
  if (result.rows[0]) return mapImageModelRow(result.rows[0]);
  const created = await query(
    `INSERT INTO image_models (
      ...
      unit_price_token_text_to_image, unit_price_token_image_to_image,
      unit_price_rmb_text_to_image, unit_price_rmb_image_to_image,
      image_billing_mode, image_cost_per_image_cny, image_fixed_profit_per_image_cny,
      image_sell_price_per_image_cny, fallback_model_id, tags, scene_description, sort_order
    ) VALUES (
    ...
    [
      modelId,
      updates.displayName || modelId,
      updates.upstreamModel || modelId,
      updates.provider || "openrouter",
      updates.enabled !== false,
      Boolean(updates.recommended),
      updates.supportsTextToImage !== false,
      Boolean(updates.supportsImageToImage),
      Boolean(updates.supportsBatch),
      Number(updates.speedScore || 80),
      Number(updates.stabilityScore || 80),
      Number(updates.qualityScore || 80),
      Number(updates.unitPriceTokenTextToImage || 0),
      Number(updates.unitPriceTokenImageToImage || 0),
      Number(updates.unitPriceRmbTextToImage || 0),
      Number(updates.unitPriceRmbImageToImage || 0),
      updates.imageBillingMode || "legacy_rmb",
      Number(updates.imageCostPerImageCny || 0),
      Number(updates.imageFixedProfitPerImageCny || 0),
      Number(updates.imageSellPricePerImageCny || 0),
      updates.fallbackModelId || "",
      JSON.stringify(updates.tags || []),
      updates.sceneDescription || "",
      Number(updates.sortOrder || 999),
    ]
  );
  return created.rows[0] ? mapImageModelRow(created.rows[0]) : null;
}
...
function estimateImageCost(model, { mode = "text_to_image", quality = "standard", n = 1 } = {}) {
  ...
  if (model.imageBillingMode === "per_image_fixed_profit") {
    const costPerImage = Number(model.imageCostPerImageCny || 0);
    const fixedProfit = Number(model.imageFixedProfitPerImageCny || 0);
    const sellPerImage = Number(model.imageSellPricePerImageCny || 0) || (costPerImage + fixedProfit);
    if (!Number.isFinite(costPerImage) || costPerImage <= 0) {
      throw imagePricingError("该图片模型缺少上游成本配置，暂无法生成", "IMAGE_MODEL_COST_NOT_CONFIGURED");
    }
    if (!Number.isFinite(sellPerImage) || sellPerImage <= 0) {
      throw imagePricingError("该图片模型缺少销售价配置，暂无法生成", "IMAGE_MODEL_PRICE_NOT_CONFIGURED");
    }
    ...
    const profitPerImage = sellPerImage - costPerImage;
    return {
      tokenCost: roundMoney(tokenBase * qualityMultiplier * count),
      moneyCost: roundMoney(sellPerImage * count),
      upstreamCostCny: roundMoney(costPerImage * count),
      sellPriceCny: roundMoney(sellPerImage * count),
      profitCny: roundMoney(profitPerImage * count),
      profitMargin: sellPerImage > 0 ? roundMoney((profitPerImage / sellPerImage) * 100) : 0,
      billingMode: "per_image_fixed_profit",
      imageCount: count,
      costPerImageCny: roundMoney(costPerImage),
      sellPricePerImageCny: roundMoney(sellPerImage),
    };
  }
  const tokenBase = mode === "image_to_image"
    ? Number(model.unitPriceTokenImageToImage || model.unitPriceTokenTextToImage || 0)
  ...
  return {
    ...
    upstreamCostCny: 0,
    sellPriceCny: roundMoney(moneyBase * qualityMultiplier * count),
    profitCny: 0,
    profitMargin: 0,
    billingMode: model.imageBillingMode || "legacy_rmb",
    imageCount: count,
    costPerImageCny: 0,
    sellPricePerImageCny: roundMoney(moneyBase * qualityMultiplier),
  };
}
...
function extractUpstreamError(payload = {}, status = 0) {
  ...
  const message = String(
    error?.message
      ...
      || payload?.message
      || payload?.raw
      || (status ? `模型服务返回 ${status}` : "图片模型服务请求失败")
  ).replace(/\s+/g, " ").slice(0, 500);
  return { code, message };
}

function friendlyUpstreamMessage(status = 0, code = "") {
  if (status === 401 || status === 403) return "图片模型服务鉴权失败，管理员需要检查配置";
  if (status === 402) return "图片模型服务额度不足，管理员需要检查账户余额";
  if (status === 404) return "当前图片模型暂不可用，请切换其他图片模型";
  if (status === 408) return "图片生成超时，系统已按规则处理且未扣费";
  if (status === 413) return "参考图片过大，请压缩后重试";
  if (status === 422 || code === "IMAGE_OUTPUT_EMPTY") return "模型服务没有返回可用图片，请换一个模型或调整描述后重试";
  if (status === 429) return "图片模型服务暂时繁忙，请稍后重试";
...
async function callImageUpstream({
  ...
  const upstream = upstreams.find((item) => item.name === "aicards")
    || upstreams.find((item) => item.name === "aheapi")
    || upstreams.find((item) => item.name === "uniapi")
    || upstreams.find((item) => item.name === "new-api")
    || upstreams.find((item) => item.name === "openrouter")
    || upstreams[0];
  if (!upstream?.apiKey) {
    return {
      ok: false,
  ...
  try {
    ...
    return {
      ...
      errorMessage: error?.name === "AbortError" ? "图片生成超时，系统将尝试自动重试" : (error?.message || "图片模型服务请求失败"),
...
async function executeImageRequest({
  ...
  const upstreams = getUpstreamConfigs();
  const preferredUpstream = upstreams.find((item) => item.name === "aicards")
    || upstreams.find((item) => item.name === "aheapi")
    || upstreams.find((item) => item.name === "uniapi")
    || upstreams.find((item) => item.name === "new-api")
    || upstreams.find((item) => item.name === "openrouter")
  ...
  if (canTryFallback && currentModel.fallbackModelId) {
    ...
    if (fallbackModel?.enabled) {
      try {
        estimateImageCost(fallbackModel, {
          mode,
          quality,
          n: requestedCount,
        });
        ...
        return {
          ...
          fallbackSkipReason: "IMAGE_MODEL_PROFIT_GUARD",
        };
      }
      fallbackUsed = true;
...
async function insertImageLog(log) {
  ...
  await query(
    `INSERT INTO image_generation_logs (
      ...
      upstream_cost_cny, sell_price_cny, profit_cny, profit_margin, billing_mode,
      image_count, cost_per_image_cny, sell_price_per_image_cny,
    ...
    [
      ...
      payload.tokenCost,
      payload.moneyCost,
      payload.upstreamCostCny || 0,
      payload.sellPriceCny || payload.moneyCost || 0,
      payload.profitCny || 0,
      payload.profitMargin || 0,
      payload.billingMode || "",
      payload.imageCount || payload.outputImageCount || 0,
      payload.costPerImageCny || 0,
      payload.sellPricePerImageCny || 0,
      payload.balanceBefore,
      payload.balanceAfter,
      payload.status,
      payload.errorCode,
...
export async function runImageStudioJob({
  ...
  let estimation;
  try {
    estimation = estimateImageCost(selectedModel, {
      mode,
      quality,
      n,
    });
    ...
    return {
      ...
      error: error.message || "该图片模型暂未配置可用商业价格，请切换其他模型或联系管理员",
      code: error.code || "IMAGE_MODEL_PRICE_NOT_CONFIGURED",
    };
  }
  ...
  if (apiKeyId) {
    const quotaCheck = await checkApiKeyLimitById(apiKeyId, {
      costCny: estimation.moneyCost,
      tokens: estimation.tokenCost,
    });
    if (!quotaCheck.ok) {
      return {
        ok: false,
        status: quotaCheck.status || 429,
        error: quotaCheck.error,
        code: quotaCheck.code || "API_KEY_QUOTA_EXCEEDED",
        requestId,
        nextResetAt: quotaCheck.nextResetAt || null,
        tokenCost: 0,
        moneyCost: 0,
      };
    }
  ...
  const imageLog = await insertImageLog({
    ...
    upstreamCostCny: actualCost.upstreamCostCny || 0,
    sellPriceCny: actualCost.sellPriceCny || actualCost.moneyCost || 0,
    profitCny: actualCost.profitCny || 0,
    profitMargin: actualCost.profitMargin || 0,
    billingMode: actualCost.billingMode || selectedModel.imageBillingMode || "legacy_rmb",
    imageCount: actualCost.imageCount || outputCount || Number(n || 1),
    costPerImageCny: actualCost.costPerImageCny || 0,
    sellPricePerImageCny: actualCost.sellPricePerImageCny || 0,
  ...
  await insertImageGenerationRecord({
    ...
    internalCost: actualCost.upstreamCostCny || upstreamResult.usage?.upstreamCost || 0,
    sellPrice: actualCost.moneyCost,
    profit: actualCost.profitCny || roundMoney(actualCost.moneyCost - Number(upstreamResult.usage?.upstreamCost || 0)),
    errorCode: upstreamResult.errorCode || "",
    errorMessage: upstreamResult.ok ? "" : (upstreamResult.errorMessage || "图片生成失败"),
    latencyMs,
  });
  ...
  if (upstreamResult.ok) {
    if (apiKeyId) {
      await recordApiKeyLimitUsage(apiKeyId, {
        costCny: actualCost.moneyCost,
        tokens: actualCost.tokenCost,
      });
    }
    import("@/lib/titles/recalculate-user-titles")
      .then(({ markUserTitlesDirty }) => markUserTitlesDirty(customerId, "image_generation"))
      .catch(() => {});
...
export async function listImageHistory({
  ...
  if (!hasDatabase()) {
    const items = imageStudioStore.imageLogs
      .map(mapImageLogRow)
      .map((item) => sanitizeImageLogForViewer(item, isAdminCustomer(context.customer)))
  ...
  return {
    workspace: context.workspace,
    role: context.role,
    items: result.rows.map(mapImageLogRow).map((item) => sanitizeImageLogForViewer(item, isAdminCustomer(context.customer))),
  };
}
...
function summarizeLogs(logs = []) {
  ...
  const modelSpend = logs.reduce((memo, item) => {
    const key = item.modelDisplayName || "Unknown";
    memo[key] = roundMoney((memo[key] || 0) + Number(item.moneyCost || 0));
    return memo;
  }, {});
  const topModelByUsage = Object.entries(modelCounter).sort((a, b) => b[1] - a[1])[0]?.[0] || "暂无";
  const topModelBySpend = Object.entries(modelSpend).sort((a, b) => b[1] - a[1])[0]?.[0] || "暂无";
  return {
