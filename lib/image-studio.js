...
function isAdminCustomer(customer = {}) {
  return customer?.role === "admin" || customer?.id === "cus_admin" || isAdminEmail(customer?.email);
}
...
function estimateImageCost(model, { mode = "text_to_image", quality = "standard", n = 1 } = {}) {
  ...
  if (model.imageBillingMode === "per_image_fixed_profit") {
    ...
    if (!Number.isFinite(costPerImage) || costPerImage <= 0) {
      throw imagePricingError("该图片模型缺少上游成本配置，暂不能生成", "IMAGE_MODEL_COST_NOT_CONFIGURED");
    }
    if (!Number.isFinite(sellPerImage) || sellPerImage <= 0) {
      throw imagePricingError("该图片模型缺少销售价配置，暂不能生成", "IMAGE_MODEL_PRICE_NOT_CONFIGURED");
    }
