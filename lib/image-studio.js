...
function estimateImageCost(model, { mode = "text_to_image", quality = "standard", n = 1 } = {}) {
  ...
  if (model.imageBillingMode === "per_image_fixed_profit") {
    ...
    const minMargin = minimumImageProfitMargin();
    if (!Number.isFinite(costPerImage) || costPerImage <= 0) {
      throw imagePricingError("该图片模型缺少上游成本配置，已阻止生成以避免亏损", "IMAGE_MODEL_COST_NOT_CONFIGURED");
    }
    if (!Number.isFinite(sellPerImage) || sellPerImage <= 0) {
      throw imagePricingError("该图片模型缺少销售价配置，已阻止生成以避免亏损", "IMAGE_MODEL_PRICE_NOT_CONFIGURED");
    }
    if (sellPerImage < costPerImage * (1 + minMargin)) {
      throw imagePricingError(`该图片模型售价未覆盖成本和最低毛利 ${Math.round(minMargin * 100)}%，已阻止生成`, "IMAGE_MODEL_MARGIN_TOO_LOW");
    }
...
function friendlyUpstreamMessage(status = 0, code = "") {
  ...
  if (status === 402) return "图片模型服务余额不足，管理员需要检查账户余额";
