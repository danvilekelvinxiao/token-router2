...
export async function isTokenWhitelisted(token) {
  ...
  // When passthrough is globally enabled, allow all unknown tokens
  // to pass through to New API without requiring a DB whitelist entry.
  if (!hasDatabase()) return PASSTHROUGH_ENABLED;
  ...
  try {
    ...
    // Table may not exist — still allow passthrough when env is on
  }

  // Global passthrough mode: allow any unknown token
  return PASSTHROUGH_ENABLED;
}
