...
function getVerifySecret() {
  const secret = String(process.env.VERIFY_SECRET || "").trim();
  if (secret) return secret;
  if (process.env.NODE_ENV !== "production") return "flowapi-verify-secret-2024";
  throw new Error("生产环境必须配置 VERIFY_SECRET");
}
