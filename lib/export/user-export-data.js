...
export async function buildBillingExport(customerId, filters) {
  ...
  const orderRows = orders.map((order) => ({
    ...
    "订单内容": "FlowAPI 余额充值",
    "支付金额（¥）": toNumber(order.paymentAmountRmb ?? order.payment_amount_rmb ?? order.amount),
    "到账金额（$ API）": statusLabel(order.status) === "已完成" ? toNumber(order.creditedAmountApi ?? order.credited_amount_api ?? order.amount) : 0,
    "赠送金额（$ API）": 0,
    "兑换码": "",
  ...
  const redeemRows = redeemLogs.map((log) => ({
    ...
    "支付金额（¥）": 0,
    "到账金额（$ API）": toNumber(log.amount),
    "赠送金额（$ API）": toNumber(log.amount),
    "兑换码": "",
    "支付渠道流水号": "",
    "状态": "已完成",
...
export async function buildPackageRecordsExport(customerId, filters) {
  const { logs } = await listActivityLogs({ customerId, limit: 5000 });
  const rows = logs
    .filter((log) => ["redeem_code", "purchase_package", "membership_bonus", "daily_bonus"].includes(log.action))
    .filter((log) => withinDateRange(log.createdAt, filters))
    .map((log) => ({
      "兑换时间": formatDateTime(log.createdAt),
      "套餐名称": log.detail?.match(/套餐[:：]\s*([^，,]+)/)?.[1] || log.detail || "套餐 / 兑换记录",
      "激活码": log.action === "redeem_code" ? "已脱敏保存" : "",
      "有效期": "",
      "到账内容": toNumber(log.amount),
      "状态": "已完成",
      "备注": log.detail || "",
    }));

  return { headers: PACKAGE_RECORD_HEADERS, rows };
}
  ...
  const rows = (customer?.calls || [])
    ...
    .map((call) => {
      ...
      return {
        ...
        "API Key": maskApiKey(apiKey.token),
        ...
        "官方预估花费（$ API）": officialCost || "",
        "FlowAPI 实际花费（$ API）": actualCost,
        "节省金额（$ API）": saved || "",
...
export async function buildModelAnalysisExport(customerId, filters, query = {}) {
  ...
  const detailRows = usage.rows.map((row) => ({
    ...
    "总 Token": row["总 Token"],
    "FlowAPI 实际花费（$ API）": row["FlowAPI 实际花费（$ API）"],
    "官方预估花费（$ API）": row["官方预估花费（$ API）"],
    "节省金额（$ API）": row["节省金额（$ API）"],
    "响应时间": row["响应时间"],
    "状态": row["请求状态"],
  }));
  ...
  for (const row of usage.rows) {
    ...
    current["官方预估花费（$ API）"] += toNumber(row["官方预估花费（$ API）"]);
    current["FlowAPI 实际花费（$ API）"] += toNumber(row["FlowAPI 实际花费（$ API）"]);
    current["节省金额（$ API）"] += toNumber(row["节省金额（$ API）"]);
    ...
    daily["官方预估花费（$ API）"] += toNumber(row["官方预估花费（$ API）"]);
    daily["FlowAPI 实际花费（$ API）"] += toNumber(row["FlowAPI 实际花费（$ API）"]);
    daily["节省金额（$ API）"] += toNumber(row["节省金额（$ API）"]);
  ...
  const costRanks = [...summaryRows].sort((a, b) => b["FlowAPI 实际花费（$ API）"] - a["FlowAPI 实际花费（$ API）"]);
  const requestRanks = [...summaryRows].sort((a, b) => b["请求次数"] - a["请求次数"]);
  summaryRows.forEach((row) => {
    row["平均单次成本（$ API）"] = row["请求次数"] ? row["FlowAPI 实际花费（$ API）"] / row["请求次数"] : 0;
    row["平均响应时间"] = "";
    row["成功率"] = row["请求次数"] ? `${(((row["请求次数"] - row["失败次数"]) / row["请求次数"]) * 100).toFixed(2)}%` : "";
    row["节省比例"] = row["官方预估花费（$ API）"] > 0 ? `${((row["节省金额（$ API）"] / row["官方预估花费（$ API）"]) * 100).toFixed(2)}%` : "";
  ...
  const dailyRows = Array.from(dailyMap.values()).map((row) => ({
    ...row,
    "节省比例": row["官方预估花费（$ API）"] > 0 ? `${((row["节省金额（$ API）"] / row["官方预估花费（$ API）"]) * 100).toFixed(2)}%` : "",
  }));
