...
import { hasDatabase, pingDatabase, query } from "./db.js";
...
export function makeFlowApiPublicModelId(modelId = "") {
  return `flowapi-${publicFamilyFromModelId(modelId)}-${stablePublicHash(modelId)}`;
}
...
function safePublicModelId(value = "", fallbackModelId = "") {
  ...
  if (requested.startsWith("flowapi-") && !containsUnsafePublicModelText(requested)) return requested;
...
function encryptText(plain = "") {
  ...
  if (!value) return "";
  const secret = process.env.ADMIN_CONFIG_ENCRYPTION_KEY
    || process.env.PROXY_ACCESS_TOKEN
    || process.env.ADMIN_SECRET
    || (process.env.NODE_ENV !== "production" ? "flowapi-local-dev-key" : "");
  if (!secret) {
    throw new Error("生产环境必须配置 ADMIN_CONFIG_ENCRYPTION_KEY / PROXY_ACCESS_TOKEN / ADMIN_SECRET");
  }
  ...
  try {
    ...
    const response = await fetch(modelsUrl, {
      headers: {
        ...
        ...(String(upstream.type || upstream.name || "").toLowerCase().includes("new api") || String(upstream.name || "").toLowerCase() === "new-api"
          ? { "New-Api-User": "1" }
          : {}),
...
function mapPublishedRow(row) {
  return {
    ...
    displayOrder: row.display_order !== null && row.display_order !== undefined ? Number(row.display_order) : undefined,
    featured: Boolean(row.featured || row.recommended || row.hot),
    providerFamily: row.provider_family || "",
    releaseDate: row.release_date || row.official_release_date || "",
...
export async function listPublishedModels({ target = "" } = {}) {
  ...
  if (!hasDatabase()) {
    ...
    return sortModelsForDisplay(rows);
  }
  const filters = [];
  ...
  return sortModelsForDisplay(result.rows.map(mapPublishedRow));
}
...
function hasModelHealthSignal(model = {}) {
  return Boolean(model.lastHealthCheckAt) && !String(model.lastError || "").trim();
}
...
export async function checkAdminSyncConsistency() {
  ...
  if (!dbOk) {
    return {
      ok: true,
      checkedAt: nowIso(),
      counts: { pricing: 0, imageModels: 0, apiKeyModelConfigs: 0 },
      checks: [
        { name: "数据库连通性", status: "manual", message: "数据库当前暂不可达，已跳过同步明细校验；这不影响线上模型调用主链路。" },
      ],
      issues: [{ level: "manual", message: "数据库当前不可用，暂时跳过商业同步明细校验" }],
    };
  }
  ...
  const published = await listPublishedModels().catch(() => []);
  published.forEach((model) => {
    const visible = model.enabled !== false && (model.showInModelSquare !== false || model.showInApiKeyCreate !== false || model.showInImageGeneration === true);
    if (visible && !hasModelHealthSignal(model)) {
      issues.push({
        level: "warn",
        item: model.modelId,
        message: "公开模型缺少最近健康检查信号，建议先完成可用性检测再对外展示。",
      });
    }
  });
