...
import { getAllModelConfigs, saveModelConfig } from "./model-store.js";
...
async function listImageModelsSafe(options = {}) {
  const { listImageModels } = await import("./image-studio.js");
  return listImageModels(options);
}

async function updateImageModelConfigSafe(modelId, updates = {}) {
  const { updateImageModelConfig } = await import("./image-studio.js");
  return updateImageModelConfig(modelId, updates);
}
...
function makeStableId(prefix, value = "") {
  const digest = crypto.createHash("sha1").update(String(value || "model")).digest("hex").slice(0, 12);
...
function stablePublicHash(value = "") {
  return crypto.createHash("sha1").update(String(value || "model")).digest("hex").slice(0, 8);
}

function publicFamilyFromModelId(modelId = "") {
  const id = String(modelId || "").toLowerCase();
...
export function makeFlowApiPublicModelId(modelId = "") {
  return `model-${publicFamilyFromModelId(modelId)}-${stablePublicHash(modelId)}`;
}

const PUBLIC_MODEL_BLOCKLIST = [
  "aicards",
  "aicards.shop",
  "openrouter",
  "uniapi",
...
function containsUnsafePublicModelText(value = "") {
  const text = String(value || "").toLowerCase();
  return PUBLIC_MODEL_BLOCKLIST.some((term) => text.includes(term));
}

function safeFlowApiDisplayName(value = "", fallbackModelId = "") {
  const original = String(value || "").trim();
  if (original && !containsUnsafePublicModelText(original)) return original;
  return displayNameFromModelId(fallbackModelId || "FlowAPI 高级模型");
}

function safePublicModelId(value = "", fallbackModelId = "") {
  const requested = String(value || "").trim();
  if (!requested) {
    return makeFlowApiPublicModelId(fallbackModelId || "model");
  }
  if (!containsUnsafePublicModelText(requested)) {
    return requested.startsWith("flowapi-") ? requested.replace(/^flowapi-/, "") : requested;
  }
  return makeFlowApiPublicModelId(fallbackModelId || requested || "model");
}

function sanitizePublicTags(value = []) {
  return (Array.isArray(value) ? value : String(value || "").split(/[,，]/))
    .map((item) => String(item || "").trim())
    .filter((item) => item && !containsUnsafePublicModelText(item));
}
...
export async function ensureCommercialConfigSchema() {
  ...
  await query(`
    CREATE TABLE IF NOT EXISTS upstream_providers (
      ...
      last_error_request_id TEXT DEFAULT '',
      last_error_status_code INTEGER NOT NULL DEFAULT 0,
      last_error_at TIMESTAMPTZ,
      last_payment_required BOOLEAN NOT NULL DEFAULT false,
      last_upstream_url TEXT DEFAULT '',
      last_api_key_preview TEXT DEFAULT '',
      remark TEXT DEFAULT '',
...
function mapUpstreamRow(row) {
  const apiKey = decryptText(row.encrypted_api_key || "");
  return {
    id: row.id,
    name: row.name,
    type: row.type,
    ...
    apiKey,
    apiKeyPreview: apiKey || row.api_key_preview || "",
    ...
    lastErrorStatusCode: Number(row.last_error_status_code || 0),
    lastErrorAt: row.last_error_at ? new Date(row.last_error_at).toISOString() : null,
    lastPaymentRequired: row.last_payment_required === true,
    lastUpstreamUrl: row.last_upstream_url || "",
    lastApiKeyPreview: row.last_api_key_preview || "",
    remark: row.remark || "",
...
export async function listUpstreams() {
  await ensureCommercialConfigSchema();
  if (!hasDatabase()) {
    return memory().upstreams.map(({ encryptedApiKey, apiKey, ...item }) => ({
      ...item,
      apiKey: apiKey || "",
      apiKeyPreview: apiKey || item.apiKeyPreview || "",
    }));
  }
...
export async function saveUpstream(input = {}, adminId = "") {
  ...
  const entry = {
    ...
    baseUrl,
    encryptedApiKey,
    apiKeyPreview: apiKey || input.apiKeyPreview || "",
    protocol,
    ...
    lastErrorRequestId: input.lastErrorRequestId || "",
    lastErrorStatusCode: Number(input.lastErrorStatusCode || 0),
    lastErrorAt: input.lastErrorAt || null,
    lastPaymentRequired: input.lastPaymentRequired === true,
    lastUpstreamUrl: input.lastUpstreamUrl || "",
    lastApiKeyPreview: input.lastApiKeyPreview || "",
    remark: input.remark || "",
  ...
  if (!hasDatabase()) {
    ...
    await query(
      `INSERT INTO upstream_providers (id, name, type, base_url, encrypted_api_key, api_key_preview, protocol, status, latency_ms, last_error, last_error_request_id, last_error_status_code, last_error_at, last_payment_required, last_upstream_url, last_api_key_preview, remark, enabled)
       ...
       ON CONFLICT (id) DO UPDATE SET
         ...
         protocol=$7,status=$8,latency_ms=$9,last_error=$10,last_error_request_id=$11,last_error_status_code=$12,last_error_at=$13,last_payment_required=$14,last_upstream_url=$15,last_api_key_preview=$16,remark=$17,enabled=$18,updated_at=NOW()`,
      [
        entry.id,
        entry.name,
        entry.type,
        ...
        entry.latencyMs,
        entry.lastError,
        entry.lastErrorRequestId,
        entry.lastErrorStatusCode,
        entry.lastErrorAt,
        entry.lastPaymentRequired,
        entry.lastUpstreamUrl,
        entry.lastApiKeyPreview,
        entry.remark,
        entry.enabled,
      ]
    );
...
async function updateUpstreamTestState(upstreamId, result) {
  ...
  await query(
    `UPDATE upstream_providers
      SET status=$2, latency_ms=$3, last_test_at=NOW(), last_error=$4, last_error_request_id=$5, last_error_status_code=$6, last_error_at=NOW(), last_payment_required=$7, last_upstream_url=$8, last_api_key_preview=$9, updated_at=NOW()
     WHERE id=$1`,
    [
      upstreamId,
      ...
      Boolean(result.paymentRequired || false),
      result.upstreamUrl || "",
...
export async function recordUpstreamFailureState(upstreamId, result = {}) {
  ...
  if (!hasDatabase()) {
    ...
    if (target) {
      target.status = "error";
      target.lastError = result.error || "";
      target.lastErrorRequestId = result.requestId || "";
      target.lastErrorStatusCode = Number(result.statusCode || 0);
      target.lastErrorAt = nowIso();
      target.lastPaymentRequired = result.paymentRequired === true;
      target.lastUpstreamUrl = result.upstreamUrl || "";
      target.lastApiKeyPreview = result.apiKeyPreview || "";
  ...
  await query(
    `UPDATE upstream_providers
     SET status='error', latency_ms=$2, last_error=$3, last_error_request_id=$4, last_error_status_code=$5,
         last_error_at=NOW(), last_payment_required=$6, last_upstream_url=$7, last_api_key_preview=$8, updated_at=NOW()
     WHERE id=$1`,
    [
      upstreamId,
...
function normalizePricingPayload(model, input = {}) {
  ...
  return {
    id: input.id || makeStableId("pricing", input.modelId || model.modelId),
...
function normalizePublishedInput(input = {}) {
  const rawModelId = String(input.modelId || input.publicModelId || input.id || "").trim();
  const upstreamModelId = String(input.upstreamModelId || input.actualModelId || input.modelId || rawModelId).trim();
  const modelId = safePublicModelId(rawModelId, upstreamModelId);
  if (!modelId) throw new Error("请填写模型 ID");
  const normalizedPublicModelId = getPublicModelRequestId(modelId);
  const normalizedUpstreamModelId = String(upstreamModelId || modelId).trim();
  const displayName = safeFlowApiDisplayName(input.displayName || modelId, upstreamModelId || modelId);
  const modelType = String(input.modelType || "text").trim();
  const tags = sanitizePublicTags(input.tags);
  return {
    id: input.id || makeId("pub"),
    importedModelId: input.importedModelId || "",
    modelId: normalizedPublicModelId,
    displayName,
    provider: "FlowAPI",
    logo: "FlowAPI",
    modelType,
...
export async function upsertPublishedModel(input = {}, adminId = "") {
  ...
  if (!hasDatabase()) {
    ...
    await query(
      ...
      [
        model.id || makeStableId("pub", model.modelId),
...
async function enableDefaultImageModelPack() {
  const retiredIds = new Set(["gpt-image-2", "flux-pro", "flux-schnell", "sdxl", "recraft", "ideogram"]);
  const models = await listImageModelsSafe({ includeDisabled: true }).catch(() => []);
  const targets = models.filter((item) => {
    const id = String(item.id || "");
    return id.startsWith("flowapi-") && !retiredIds.has(id);
  });
  const enabled = [];
  const skipped = [];
  ...
  for (const model of targets) {
    try {
      if (!model.enabled) {
        await updateImageModelConfigSafe(String(model.id), { enabled: true });
      }
      enabled.push({
        id: model.id,
      ...
      skipped.push({
        id: model.id,
        displayName: model.displayName || model.display_name || model.id,
        reason: error.message || "图片模型启用失败",
      });
    }
  }

  const latest = await listImageModelsSafe({ includeDisabled: true }).catch(() => []);
  return {
    enabled,
    enabledCount: enabled.length,
    skipped,
    skippedCount: skipped.length,
    totalEnabled: latest.filter((item) => item.enabled).length,
  };
}

function buildModelPackAcceptance({ published = [], skipped = [], syncResult = {}, imagePack = null } = {}) {
  const publishedCount = Number(published.length || 0);
  const apiKeyCount = published.filter((item) => item.enabled !== false && item.showInApiKeyCreate !== false).length;
  const modelSquareCount = published.filter((item) => item.enabled !== false && item.showInModelSquare !== false).length;
  const imageCount = Number(imagePack?.totalEnabled || 0);
  const blockingIssues = (syncResult?.issues || []).filter((item) => item.level === "error");
  const warnings = [
    ...skipped.map((item) => `${item.displayName || item.id}：${item.reason || "未开通"}`),
    ...(imagePack?.skipped || []).map((item) => `${item.displayName || item.id}：${item.reason || "图片模型未启用"}`),
    ...(syncResult?.issues || []).filter((item) => item.level !== "error").map((item) => item.message || item.item || "同步提醒"),
  ].filter(Boolean);

  return {
    ok: publishedCount > 0 && apiKeyCount > 0 && modelSquareCount > 0 && blockingIssues.length === 0,
    readyForApiKey: apiKeyCount > 0,
    readyForModelSquare: modelSquareCount > 0,
    readyForImages: imageCount > 0,
    modelSquareCount,
    apiKeyCount,
    imageModelCount: imageCount,
    publishedCount,
    skippedCount: skipped.length + Number(imagePack?.skippedCount || 0),
    summary: publishedCount > 0
      ? `已开通 ${publishedCount} 个文字模型，模型广场 ${modelSquareCount} 个可展示，API Key ${apiKeyCount} 个可选择，图片模型 ${imageCount} 个可用。`
      : "这次没有开通文字模型，请检查模型成本、售价或上游模型 ID。",
    nextStep: apiKeyCount > 0
      ? "现在可以去 API 管理创建一个测试 Key，再用前台模型 ID 做一次真实调用。"
      : "请先处理跳过原因，至少开通 1 个可创建 API Key 的模型。",
    blockingIssues,
    warnings,
  };
}
...
export async function bootstrapModelMarketPack({ pack = "starter", adminId = "" } = {}) {
  ...
  const imagePack = pack === "full"
    ? await enableDefaultImageModelPack().catch((error) => ({
      enabled: [],
      enabledCount: 0,
      skipped: [{ id: "image-models", reason: error.message || "默认图片模型启用失败" }],
      skippedCount: 1,
      totalEnabled: 0,
    }))
    : null;
  const syncResult = await checkAdminSyncConsistency().catch((error) => ({ ok: false, issues: [{ level: "error", message: error.message }] }));
  const acceptance = buildModelPackAcceptance({ published, skipped, syncResult, imagePack });
  await writeAdminAuditLog({
    adminId,
    action: "bootstrap_model_market_pack",
    targetType: "model_pack",
    targetId: pack,
    after: { pack, publishedCount: published.length, skipped, imagePack, acceptance },
    syncResult,
  });
...
export async function publishModels({ importedModelIds = [], modelSettings = {}, pricing = {}, adminId = "" } = {}) {
  ...
  for (const model of targets) {
    ...
    const requestedPublicId = String(settings.publicModelId || "").trim();
    const publicId = safePublicModelId(requestedPublicId, model.modelId);
    const displayName = safeFlowApiDisplayName(settings.displayName || model.displayName, model.modelId);
    const modelType = settings.modelType || model.modelType;
    const tags = sanitizePublicTags(settings.tags);
    const publishEntry = {
      id: makeStableId("pub", publicId),
    ...
    if (modelType === "image" || modelType === "image-edit") {
      await updateImageModelConfigSafe(publicId, {
        displayName,
...
export async function checkAdminSyncConsistency() {
  ...
  const [pricing, imageModels, modelConfigs] = await Promise.all([
    ...
    listImageModelsSafe({ includeDisabled: true }).catch(() => []),
