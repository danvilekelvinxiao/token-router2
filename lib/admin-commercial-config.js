...
export async function syncUpstreamModels(upstreamId) {
  const upstream = await getUpstream(upstreamId, { includeSecret: true });
  if (!upstream) throw new Error("上游不存在");
  const response = await fetch(`${normalizeBaseUrl(upstream.baseUrl)}/v1/models`, {
    headers: {
      Authorization: `Bearer ${upstream.apiKey}`,
      "Content-Type": "application/json",
    },
  ...
  const text = await response.text();
  if (!response.ok) {
    throw new Error(`拉取模型失败：HTTP ${response.status} ${text.slice(0, 200)}`);
  }
  let data = null;
  try { data = JSON.parse(text); } catch {
  ...
  if (!hasDatabase()) {
    ...
    await query(
      `INSERT INTO published_models (
        id, imported_model_id, model_id, display_name, provider, logo, model_type,
      ...
      ON CONFLICT (model_id) DO UPDATE SET
        display_name=$4, provider=$5, logo=$6, model_type=$7, description=$8, tags_json=$9::jsonb,
        official_release_date=$10, sort_order=$11, recommended=$12, hot=$13, free=$14,
        member_only=$15, black_gold_only=$16, enabled=$17, show_in_model_square=$18,
        show_in_api_key_create=$19, show_in_image_generation=$20, updated_at=NOW()`,
      [
        model.id,
...
export async function publishModels({ importedModelIds = [], modelSettings = {}, pricing = {}, adminId = "" } = {}) {
  ...
  for (const model of targets) {
    ...
    const pricePayload = await saveModelPricing(
      ...
      pricing[model.id] || pricing[model.modelId] || { billingMode: modelType === "image" || modelType === "image-edit" ? "per_image_fixed_profit" : "token_multiplier" }
    );
