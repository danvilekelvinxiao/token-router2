  ...
  if (!initPromise) {
    initPromise = activePool.query(`
      ...
      INSERT INTO customers
        (id, name, phone, company, email, password_hash, my_invite_code, balance, total_spend, email_verified)
      VALUES
        ('cus_admin', 'xiaoyijie', '', 'FlowAPI', 'xiaoyijie@flowapi.fun', 'xiaoyijie2', '0001', 9999, 0, true)
      ON CONFLICT (id) DO NOTHING;

      INSERT INTO api_keys
        (id, customer_id, token, label)
      VALUES
        ('key_admin', 'cus_admin', 'tr_admin_${Date.now().toString(36)}', '管理员 API 密匙')
      ON CONFLICT (id) DO NOTHING;
    `);
  }
  await initPromise;
