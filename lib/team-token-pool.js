...
function normalizeModelMatchList(value = "") {
  return [...new Set(normalizeList(value).map((item) => normalizeModelMatchKey(item)).filter(Boolean))];
}
...
function rowToToken(row, { includeSecret = false } = {}) {
  const token = {
    ...
    allowedModels: normalizeModelMatchList(row.allowed_models || ""),
...
function normalizeTokenInput(input = {}) {
  ...
  return {
    ...
    allowedModels: normalizeModelMatchList(input.allowedModels),
...
function tokenMatches(token, context) {
  ...
  const model = normalizeModelMatchKey(context.model || "");
...
function cacheEnabledFor(settings, context) {
  ...
  if (settings.models.length && model && !settings.models.map((item) => normalizeModelMatchKey(item)).includes(model)) return false;
