...
function normalizeApiPath(value = "") {
  const path = String(value || "/v1/chat/completions").trim();
  if (!path) return "/v1/chat/completions";
  return path.startsWith("/") ? path : `/${path}`;
}

export function buildTokenPoolUpstreamUrl(baseUrl = "", apiPath = "/v1/chat/completions") {
...
export function decryptTokenSecret(record = {}) {
  const fallback = sanitizeSecretText(record.tokenPreview || record.token_preview || "");
  if (!record.tokenCiphertext || !record.tokenIv || !record.tokenAuthTag) return fallback;
  try {
...
function rowToToken(row, { includeSecret = false } = {}) {
  const token = {
    ...
    apiPath: normalizeApiPath(row.api_path || "/v1/chat/completions"),
...
function normalizeTokenInput(input = {}) {
  ...
  return {
    ...
    apiPath: normalizeApiPath(input.apiPath || "/v1/chat/completions"),
    encrypted,
  };
}
...
export async function selectTeamTokenCandidatesForRequest(context = {}) {
  await ensureTeamTokenPoolSchema();
  const tokens = await listTokenPool();
  ...
  for (const selected of candidates) {
    ...
    if (hasDatabase()) {
      ...
      if (result.rows[0]) secret = rowToToken(result.rows[0], { includeSecret: true }).secret || "";
    } else {
      const record = memory.tokens.find((item) => item.id === selected.id);
      if (record) secret = decryptTokenSecret(record);
    }
    if (secret) selectedCandidates.push({ ...selected, secret });
  }
  return selectedCandidates;
}

export async function selectTeamTokenForRequest(context = {}) {
  const candidates = await selectTeamTokenCandidatesForRequest(context);
...
export async function testTokenPoolToken({ tokenId, adminId = "", useRealUpstream = false } = {}) {
  ...
  try {
    ...
    if (useRealUpstream) {
      const upstreamUrl = buildTokenPoolUpstreamUrl(record.baseUrl, record.apiPath);
