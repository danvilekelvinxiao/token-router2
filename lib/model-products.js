...
const GPT55_ACTUAL_MODEL =
  process.env.SUB2API_MODEL_GPT55 ||
  process.env.FLOWAPI_GPT55_ACTUAL_MODEL ||
  process.env.FLOWAPI_UNIAPI_GPT55_ACTUAL_MODEL ||
  "gpt-5.5";
...
export const MODEL_PRODUCTS = [
  ...
  {
    id: "flowapi-claude-opus-48",
    displayName: "Claude Opus 4.8",
    publicModelId: "flowapi-claude-opus-48",
    actualModelId: CLAUDE_OPUS_48_ACTUAL_MODEL,
    group: "claude-premium",
    executionGroup: process.env.NEW_API_CLAUDE_EXECUTION_GROUP || process.env.NEW_API_OPENAI_EXECUTION_GROUP || "claude-premium",
    planId: "claude-premium",
    description: "适合最新 Opus 路线验证、深度分析和高难度 Agent 任务。",
    useCases: ["最新 Opus 验证", "深度分析", "Agent 任务", "高难度任务"],
    recommendedTools: ["CC-Switch", "Cursor", "Claude Code"],
    priceMultiplier: 4.0,
    isAvailable: CLAUDE_OPUS_48_AVAILABLE,
    isComingSoon: !CLAUDE_OPUS_48_AVAILABLE,
    sortOrder: 58,
    provider: "Anthropic",
    underlyingRoute: CLAUDE_OPUS_48_AVAILABLE ? CLAUDE_OPUS_48_ACTUAL_MODEL : "上游未开通",
  },
  {
  ...
  {
    id: "flowapi-claude-opus-46",
    displayName: "Claude Opus 4.6",
    publicModelId: "flowapi-claude-opus-46",
    actualModelId: CLAUDE_OPUS_46_ACTUAL_MODEL,
    group: "claude-premium",
    executionGroup: process.env.NEW_API_CLAUDE_EXECUTION_GROUP || process.env.NEW_API_OPENAI_EXECUTION_GROUP || "claude-premium",
    planId: "claude-premium",
    description: "适合旧版 Opus 兼容验证、高质量复杂任务和回归排查。",
    useCases: ["旧版兼容", "回归排查", "复杂任务", "高质量写作"],
    recommendedTools: ["CC-Switch", "Cursor", "Claude Code"],
    priceMultiplier: 3.8,
    isAvailable: CLAUDE_OPUS_46_AVAILABLE,
    isComingSoon: !CLAUDE_OPUS_46_AVAILABLE,
    sortOrder: 62,
    provider: "Anthropic",
    underlyingRoute: CLAUDE_OPUS_46_AVAILABLE ? CLAUDE_OPUS_46_ACTUAL_MODEL : "上游未开通",
  },
  {
    id: "flowapi-gpt55",
    displayName: "GPT-5.5",
    publicModelId: "flowapi-gpt55",
    actualModelId: GPT55_ACTUAL_MODEL,
    group: "gpt-premium",
    ...
    pricing: {
      billingMode: "token_multiplier",
      inputCostPerMTokens: 6.4,
      outputCostPerMTokens: 25.6,
...
export function normalizeModelProduct(product) {
  ...
  const requestModelId = getPublicModelRequestId(validated.publicModelId || validated.id || validated.actualModelId || validated.displayName || "");
  return {
    ...validated,
    requestModelId,
    displayName: getPublicModelDisplayName(validated.displayName || validated.name || requestModelId, validated.displayName || validated.name || requestModelId),
...
export function getModelProduct(idOrModel) {
  ...
  const product = MODEL_PRODUCTS.find((item) => {
    return [item.id, item.publicModelId, item.actualModelId, item.displayName, getPublicModelRequestId(item.id), getPublicModelRequestId(item.publicModelId)]
      .filter(Boolean)
      .some((value) => {
