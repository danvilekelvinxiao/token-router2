...
export function buildDashboardOverview(customer = {}) {
  ...
  const cacheStats = {
    hitRate: calls.length ? (cachedCalls.length / calls.length) * 100 : 0,
    hitCount: cachedCalls.length,
    savedTokens: cachedCalls.reduce((total, call) => total + Number(call.savedTokens || Math.round(Number(call.tokens || 0) * 0.35)), 0),
    savedCostCny: cachedCalls.reduce((total, call) => total + Number(call.savedCostCny || Number(call.cost || 0) * 0.35), 0),
    avgLatencyImprovement: cachedCalls.length ? 32 : 0,
  };
