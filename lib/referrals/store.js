...
export async function getReferralMe(customerId) {
  ...
  const validInvites = records.items.filter((item) => item.firstRecharge || Number(item.totalRechargeCny || 0) > 0).length;
...
export async function listReferralRecords(customerId, { limit = 20 } = {}) {
  ...
  const items = result.rows.map(rowToRelation).map((item) => {
    ...
    return {
      ...
      firstRecharge: Boolean(item.firstRechargeAt) || Number(item.totalRechargeCny || 0) > 0,
      ...
      status: item.status === "suspicious" ? "异常待审" : Number(item.totalRechargeCny || 0) > 0 ? "已结算" : "待首充",
    };
  });
...
export async function handlePaidReferralOrder({ orderId, userId, paidAmountCny }) {
  ...
  const validCountResult = await query(
    `SELECT COUNT(*)::int AS cnt
     ...
     WHERE referrer_user_id = $1 AND first_recharge_at IS NOT NULL AND status <> 'invalid'`,
