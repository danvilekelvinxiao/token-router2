...
import { addWalletReferralBonus } from "@/lib/wallet/ledger";
...
export async function handlePaidReferralOrder({ orderId, userId, paidAmountCny }) {
  ...
  if (reward.creditBonusCny > 0) {
    await addWalletReferralBonus({
      userId: relation.referrer_user_id,
      tokenAmount: reward.creditBonusCny,
      type: "referral_bonus_inviter",
      relatedUserId: userId,
      relatedOrderId: orderId,
      description: `邀请奖励：拉新充值 ¥${Number(amount).toFixed(2)}`,
    });
  }
  if (reward.friendBonusCny > 0) {
    await addWalletReferralBonus({
      userId,
      tokenAmount: reward.friendBonusCny,
      type: "referral_bonus_invitee",
      relatedUserId: relation.referrer_user_id,
      relatedOrderId: orderId,
      description: `新人奖励：充值返 Token`,
    });
  }
  await query(
    `UPDATE referral_relations
