...
export async function requireAdmin(req, res) {
  ...
  if (adminSecret && providedAdminSecret && String(providedAdminSecret) === String(adminSecret)) {
    return {
      session: { customerId: "admin-secret" },
      customer: { id: "admin-secret", email: "admin@flowapi.local", role: "admin" },
    };
  }
...
export function requireCron(req, res) {
  ...
  if (cronSecret && providedCronSecret && String(providedCronSecret) === String(cronSecret)) {
    return {
      id: "cron",
      email: "cron@flowapi.local",
      role: "cron",
    };
  }

  res.status(401).json({ error: "定时任务密钥无效" });
  return null;
}
