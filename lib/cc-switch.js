...
function normalizeFlowApiEndpoint(baseUrl) {
  const normalized = normalizePublicApiBaseUrl(baseUrl || "https://flowapi.fun/v1");
...
export function buildCcSwitchUrl({ apiKey, baseUrl, model, name = "FlowAPI", displayName, enabled = true }) {
  const publicBaseUrl = normalizeFlowApiEndpoint(baseUrl || "https://flowapi.fun/v1");
