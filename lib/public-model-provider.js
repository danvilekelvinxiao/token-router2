...
function normalizePublicModelKey(value = "") {
  return String(value || "").trim().toLowerCase().replace(/\s+/g, " ");
}

export function dedupePublicModelList(models = []) {
  const seenRequestIds = new Set();
  const seenDisplayNames = new Set();
  const result = [];
  for (const model of Array.isArray(models) ? models : []) {
    const requestId = normalizePublicModelKey(getPublicModelRequestId(model?.requestModelId || model?.publicModelId || model?.modelId || model?.id || ""));
    const displayName = normalizePublicModelKey(sanitizePublicText(model?.displayName || model?.name || requestId));
    if ((requestId && seenRequestIds.has(requestId)) || (displayName && seenDisplayNames.has(displayName))) continue;
...
export function sanitizePublicModelForClient(model = {}) {
  ...
  const requestModelId = getPublicModelRequestId(sanitized.publicModelId || sanitized.modelId || sanitized.id || "");
  const displayName = getPublicModelDisplayName(sanitized.displayName || sanitized.name || requestModelId, sanitized.displayName || sanitized.name || requestModelId);
  ...
  const publicModel = {
    ...
    requestModelId,
    displayName: sanitizePublicText(displayName),
    name: sanitized.name ? sanitizePublicText(displayName) : sanitized.name,
