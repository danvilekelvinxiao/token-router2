...
export function dedupePublicModelList(models = []) {
  ...
  for (const model of Array.isArray(models) ? models : []) {
    const rawModelId = model?.requestModelId || model?.publicModelId || model?.modelId || model?.id || "";
    const key = getPublicModelRequestId(rawModelId);
    const altKeys = [
      rawModelId,
      model?.modelId,
      model?.publicModelId,
    ...
    seen.add(key);
    altKeys.forEach((alt) => seen.add(getPublicModelRequestId(alt)));
    result.push(model);
  }
  return result;
}
