...
export function sanitizePublicModelForClient(model = {}) {
  ...
  const displayName = normalizeModelDisplayLabel(sanitized);
  ...
  const publicModel = {
    ...
    displayName: sanitizePublicText(displayName || sanitized.displayName || sanitized.name || sanitized.modelId || ""),
    name: sanitized.name ? sanitizePublicText(displayName || sanitized.name) : sanitized.name,
    ...
    providerFamily: sanitized.providerFamily,
    releaseDate: sanitized.releaseDate,
    popularityScore: sanitized.popularityScore,
...
export function toPublicModelCatalogItem(model = {}) {
  const id = String(model.id || model.publicModelId || model.modelId || "").trim();
  const modelId = String(model.actualModelId || model.upstreamModelId || model.modelId || model.publicModelId || id).trim();
