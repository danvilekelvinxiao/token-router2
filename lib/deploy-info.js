...
export function getDeployInfo() {
  ...
  const fileCommit = envFileValue(["FLOWAPI_DEPLOY_COMMIT", "VERCEL_GIT_COMMIT_SHA", "GITHUB_SHA"]);
  const fileBranch = envFileValue(["FLOWAPI_DEPLOY_BRANCH", "VERCEL_GIT_COMMIT_REF"]);
  const commit =
    gitCommit ||
    fileCommit ||
