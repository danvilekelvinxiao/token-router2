...
import { createNewApiToken } from "./new-api/client";
...
async function publicCustomer(customer) {
  ...
  const [keysResult, callsResult, inviteResult] = await Promise.all([
    query("SELECT * FROM api_keys WHERE customer_id = $1 ORDER BY created_at ASC", [customer.id]),
...
export async function registerCustomer({ email, password, inviteCode = "" }) {
  ...
  if (!hasDatabase()) {
    ...
    if (customer) {
      if (password) customer.passwordHash = password;
      ...
      await createApiKey(customer.id, "默认 API 密匙");
    }
  ...
  if (customer) {
    await query(
      ...
      [customer.id, password || null, inviteResult.inviteCode || "", inviteResult.inviterId || null]
    );
    customer = {
      ...customer,
    ...
    const inserted = await query(
      `INSERT INTO customers
       ...
       RETURNING *`,
      [id, cleanEmail.split("@")[0], cleanEmail, password, inviteResult.inviteCode || "", inviteResult.inviterId || null]
    );
...
export async function loginCustomer({ email, password }) {
  ...
  if (!hasDatabase()) {
    const customer = store.customers.find((item) => item.email === cleanEmail && item.passwordHash === password);
    if (!customer) return null;
  ...
  const result = await query("SELECT * FROM customers WHERE email = $1 AND password_hash = $2 LIMIT 1", [cleanEmail, password]);
...
export async function createApiKey(customerId, label = "API 密匙", expiresAt = null) {
  let newApiToken;
  try {
    ...
    const message = error?.message || "New API 令牌创建失败";
    throw new Error(`API 密钥创建失败，请稍后重试或联系管理员。${message ? `（${message}）` : ""}`);
  }

  const realKey = String(newApiToken?.key || "").trim();
  if (!realKey.startsWith("sk-")) {
    throw new Error("API 密钥创建异常：New API 返回的 Key 不是 sk- 开头，已阻止导入 CC-Switch。");
  }
  ...
  const result = await query(
    `INSERT INTO api_keys
     ...
     RETURNING *`,
    [key.id, customerId, key.token, key.newApiId, key.newApiSyncStatus, label, key.expiresAt]
  );
  logActivity({ customerId, action: "create_key", category: "api_key", detail: `创建 API 密匙: ${label}` });
...
export async function deleteApiKey(customerId, keyId) {
  ...
  if (!hasDatabase()) {
    ...
    store.apiKeys = store.apiKeys.filter((item) => !(item.id === keyId && item.customerId === customerId));
    if (store.apiKeys.length === before) return null;
    return publicCustomer(store.customers.find((item) => item.id === customerId));
  }
...
export async function listCustomers() {
  if (hasDatabase()) {
    ...
    if (r) return r.rows.map(rowToCustomer);
  }
...
export async function disableApiKey(keyId) {
  if (hasDatabase()) {
    await query("UPDATE api_keys SET disabled_at = NOW() WHERE id = $1", [keyId]);
  }
  const store = memoryStore();
  const key = store.apiKeys.find((k) => k.id === keyId);
  if (key) key.disabledAt = new Date().toISOString();
  return { ok: true };
}

export async function enableApiKey(keyId) {
  if (hasDatabase()) {
    await query("UPDATE api_keys SET disabled_at = NULL WHERE id = $1", [keyId]);
  }
  const store = memoryStore();
  const key = store.apiKeys.find((k) => k.id === keyId);
  if (key) key.disabledAt = null;
  return { ok: true };
}
