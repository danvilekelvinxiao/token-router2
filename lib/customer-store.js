  ...
  apiKeys: [
    {
      ...
      token: "tr_admin_" + Date.now().toString(36),
      label: "管理员 API 密匙",
      createdAt: new Date().toISOString(),
      lastUsedAt: null,
    },
...
function signToken(payload) {
  ...
  const sig = crypto.createHmac("sha256", VERIFY_SECRET).update(b64).digest("base64url");
  return `${b64}.${sig}`;
}
...
function unsignToken(token) {
  try {
    ...
    const expected = crypto.createHmac("sha256", VERIFY_SECRET).update(b64).digest("base64url");
...
export async function createApiKey(customerId, label = "API 密匙", expiresAt = null) {
  ...
  const result = await query(
    "INSERT INTO api_keys (id, customer_id, token, label, expires_at) VALUES ($1, $2, $3, $4, $5) RETURNING *",
    [key.id, customerId, key.token, label, key.expiresAt]
  );
  logActivity({ customerId, action: "create_key", category: "api_key", detail: `创建 API 密匙: ${label}` });
