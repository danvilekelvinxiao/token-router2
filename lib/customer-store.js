...
export async function createApiKey(customerId, label = "API 密匙", expiresAt = null, modelProductInput = null) {
  ...
  if (!modelProduct.isAvailable) {
    const error = new Error("该模型上游暂未开放，请选择其他模型。");
    error.type = "model_coming_soon";
    throw error;
  }

  if (!modelProduct.actualModelId || String(modelProduct.actualModelId).trim() === "") {
    const error = new Error("该模型的上游路由未配置 (actual_model_id 缺失)，请等待管理员配置后再创建 Key。");
  ...
  try {
    newApiToken = await createNewApiToken({
      ...
      group: modelProduct.executionGroup || process.env.NEW_API_EXECUTION_GROUP || process.env.NEW_API_DEFAULT_GROUP || "default",
  ...
  const canUseServerManagedKey = Boolean(process.env.OPENROUTER_API_KEY || process.env.NEW_API_KEY);
  const newApiRawKey = String(newApiToken?.key || "").trim();
  if (!newApiRawKey && !canUseServerManagedKey) {
    const message = newApiError?.message || "New API API Key 创建失败";
    const createError = new Error(`API 密钥创建失败，请稍后重试或联系管理员。${message ? `（${message}）` : ""}`);
    createError.code = "NEW_API_TOKEN_CREATE_FAILED";
    throw createError;
  }

  if (newApiRawKey && !newApiRawKey.startsWith("sk-")) {
    const formatError = new Error("New API 返回的 API Key 格式异常");
    formatError.code = "INVALID_NEW_API_TOKEN_FORMAT";
    throw formatError;
  }

  const isServerManagedKey = !newApiRawKey;
  const managedKey = isServerManagedKey ? makeToken() : newApiRawKey;
  const key = {
    id: makeId("key"),
    customerId,
    token: managedKey,
    ...
    newApiSyncStatus: isServerManagedKey ? "server_managed" : "synced",
    newApiSyncedAt: isServerManagedKey ? null : new Date().toISOString(),
    createdAt: new Date().toISOString(),
    lastUsedAt: null,
...
export async function checkApiKeyLimitById(apiKeyId, usage = {}) {
  ...
  const costCny = roundUsage(usage.costCny || usage.cost || 0);
  const tokens = Math.max(0, Math.round(numberOrZero(usage.tokens)));

  if (!hasDatabase()) {
    const key = store.apiKeys.find((item) => item.id === apiKeyId && !item.deletedAt);
    if (!key) return { ok: true };
    const limitEnabled = Boolean(key.limitEnabled);
    const limitAmount = numberOrZero(key.limitAmount);
    if (!limitEnabled || limitAmount <= 0) return { ok: true, apiKey: key };
    const unit = key.limitUnit === "token" ? "token" : "cny";
    const used = unit === "token" ? numberOrZero(key.totalUsedTokens) : numberOrZero(key.totalUsedCny);
    const add = unit === "token" ? tokens : costCny;
    if (used + add > limitAmount) {
      return {
        ok: false,
        status: 429,
        code: "API_KEY_QUOTA_EXCEEDED",
        error: "该 API Key 已达到额度限制，请调整额度后再调用。",
        usage: roundUsage(used + add, unit === "token" ? 0 : 6),
        limitAmount,
        limitUnit: unit,
      };
    }
  ...
  const result = await query("SELECT id FROM api_keys WHERE id = $1 AND deleted_at IS NULL LIMIT 1", [apiKeyId]);
  return result.rows[0] ? { ok: true } : { ok: true };
}
...
export async function recordApiKeyLimitUsage(apiKeyId, { costCny = 0, tokens = 0 } = {}) {
  ...
  if (!hasDatabase()) {
    ...
    return {
      ok: true,
      totalUsedCny: key.totalUsedCny,
      totalUsedTokens: key.totalUsedTokens,
    };
  }
...
export async function findCustomerByToken(token) {
  ...
  const result = await query(
    `SELECT
       ...
       k.allowed_models, k.price_multiplier,
       c.*
     FROM api_keys k
...
export async function recordCallByToken(token, record) {
  ...
  if (!hasDatabase()) {
    ...
    match.customer.balance = Number((match.customer.balance - cost).toFixed(6));
  ...
  const updatedCustomer = await query(
    "UPDATE customers SET balance = balance - $2, total_spend = total_spend + $2 WHERE id = $1 RETURNING *",
    [match.customer.id, cost]
  );
  await query(
    `INSERT INTO calls
      (id, customer_id, api_key_id, customer, endpoint, requested_model, routed_model, provider, status,
       prompt_tokens, completion_tokens, tokens, cost,
       public_model_id, actual_model_id, upstream_channel, upstream_cost, profit, latency_ms, error_code, error_message)
...
export async function finalizeReservedCallByToken(token, record, reservation = 0) {
  ...
  if (!hasDatabase()) {
    if (delta < 0) {
      ...
      match.customer.balance = Number((match.customer.balance - paidDelta).toFixed(6));
