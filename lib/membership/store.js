...
export function consumeMemberQuotaTokens(userId, tokenAmount = 0) {
  const amount = Math.max(0, Number(tokenAmount || 0));
  if (!userId || amount <= 0) {
    return { consumed: 0, remaining: 0, membership: getUserMembership(userId) };
  }

  const membership = store.users[userId];
  if (!membership || membership.status !== "active") {
    return { consumed: 0, remaining: 0, membership: getUserMembership(userId) };
  }

  const available = Math.max(0, Number(membership.memberQuotaTokens || 0));
  const consumed = Math.min(available, amount);
  membership.memberQuotaTokens = Number((available - consumed).toFixed(6));
  membership.updatedAt = new Date().toISOString();

  return {
    consumed,
    remaining: membership.memberQuotaTokens,
    membership: getUserMembership(userId),
  };
}

export function refundMemberQuotaTokens(userId, tokenAmount = 0) {
  const amount = Math.max(0, Number(tokenAmount || 0));
  if (!userId || amount <= 0) {
    return { refunded: 0, remaining: 0, membership: getUserMembership(userId) };
  }
  ...
  const current = Math.max(0, Number(membership.memberQuotaTokens || 0));
  membership.memberQuotaTokens = Number((current + amount).toFixed(6));
  membership.updatedAt = new Date().toISOString();
