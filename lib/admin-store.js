...
function normalizeFallbackOrder(value = []) {
  ...
  const order = unique.length ? unique : [...DEFAULT_ROUTE_POLICY.fallbackOrder];
  if (!order.includes("newApi")) {
    const sub2Index = order.indexOf("sub2api");
    if (sub2Index >= 0) order.splice(sub2Index + 1, 0, "newApi");
    else order.unshift("newApi");
  }
  return order;
}
