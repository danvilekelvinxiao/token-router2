...
import { upsertPublishedModel } from "./admin-commercial-config";
...
function detectModelFamily(modelId = "") {
  ...
  if (id.includes("claude")) return { category: "claude", displayName: "Claude 高级通道", publicModelId: "flowapi-claude-pro" };
  if (id.includes("codex")) return { category: "codex", displayName: "Codex Plus", publicModelId: "flowapi-codex-plus" };
  if (id.includes("gemini")) return { category: "gemini", displayName: "Gemini Pro", publicModelId: "flowapi-gemini-pro" };
  if (id.includes("deepseek")) return { category: "deepseek", displayName: "DeepSeek Chat", publicModelId: id.includes("reasoner") ? "deepseek-reasoner" : "deepseek-chat" };
  if (id.includes("qwen")) return { category: "qwen", displayName: "Qwen 中文通道", publicModelId: "flowapi-qwen-pro" };
  if (id.includes("gpt") || id.includes("openai") || /\bo[134]\b/.test(id)) return { category: "chatgpt", displayName: "ChatGPT 高级通道", publicModelId: "flowapi-chatgpt-pro" };
  return { category: "general", displayName: "FlowAPI 高级通道", publicModelId: `flowapi-${sanitizeId(modelId).toLowerCase().replace(/[:._]+/g, "-").slice(0, 80)}` };
}
    ...
    await query(
      `UPDATE upstream_channels
       SET success_rate = $2,
           ...
           updated_at = NOW()
       WHERE provider_key = $1 OR provider_name = $1`,
      [
        AICARDS_PROVIDER_KEY,
        ok ? 0.98 : 0,
