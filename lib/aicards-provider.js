...
function normalizePricePair({ inputCost, outputCost, inputSell, outputSell }) {
  return {
    inputCost: Number(inputCost || 0),
    outputCost: Number(outputCost || 0),
    inputSell: Number(inputSell || 0),
    outputSell: Number(outputSell || 0),
  };
}
...
function buildAicardsAutoPricing(model = {}, options = {}) {
  ...
  normalizePricePair({ inputCost, outputCost, inputSell, outputSell });
...
export async function getAicardsConfig() {
  ...
  const envApiKey = String(process.env.AICARDS_API_KEY || "").trim();
  ...
  return {
    ...
    enabled: Boolean(baseUrl && isUsableSecret(apiKey)),
  };
}
...
export async function ensureAicardsProviderSchema() {
  ...
  await query(`
    ...
    CREATE TABLE IF NOT EXISTS model_routes (
      ...
      min_profit_margin NUMERIC(8, 4) NOT NULL DEFAULT 0,
  ...
  await query(
    `INSERT INTO model_routes (
     ...
     ON CONFLICT (id) DO UPDATE SET
       ...
       requires_admin_review = EXCLUDED.requires_admin_review,
       updated_at = NOW()`,
    [`route_${sanitizeId(publicModelId)}`, publicModelId, displayName, modelType, inputSell, outputSell, 0, publish, enable, !enable],
  );
  await query(
    `INSERT INTO route_candidates (
       id, public_model_id, upstream_channel_id, actual_model_id, provider_key, priority, weight, is_enabled, updated_at
...
function validateBulkPublishCandidate(model = {}) {
  ...
  normalizePricePair({ inputCost, outputCost, inputSell, outputSell });
  return { inputCost, outputCost, inputSell, outputSell };
}
  ...
  for (const model of targets) {
    try {
      ...
      const price = autoPrice
        ? buildAicardsAutoPricing(model, { sellMultiplier })
