...
import { invalidateRouteCaches } from "./cache-manager.js";
import { getUpstream, listPublishedModels, listUpstreams, upsertPublishedModel } from "./admin-commercial-config.js";

export const AICARDS_PROVIDER_KEY = "aicards";
export const AICARDS_CLAUDE_PROVIDER_KEY = "aicards-claude";
export const AICARDS_CHANNEL_NAME = "AICards-Backup";
export const AICARDS_CLAUDE_CHANNEL_NAME = "AICards-Claude";
export const AICARDS_GROUP_NAME = "aicards-backup";
export const AICARDS_CLAUDE_GROUP_NAME = "aicards-claude";
export const AICARDS_PROVIDER_KEYS = [AICARDS_PROVIDER_KEY, AICARDS_CLAUDE_PROVIDER_KEY];
export const AICARDS_CHANNEL_NAMES = [AICARDS_CHANNEL_NAME, AICARDS_CLAUDE_CHANNEL_NAME];
...
function isClaudeModelId(value = "") {
  return String(value || "").toLowerCase().includes("claude");
}

function familyFromModelId(modelId = "") {
  return isClaudeModelId(modelId) ? "claude" : "default";
}

function getAicardsProfileForModel(modelId = "") {
  if (familyFromModelId(modelId) === "claude") {
    return {
      family: "claude",
      providerKey: AICARDS_CLAUDE_PROVIDER_KEY,
      channelName: AICARDS_CLAUDE_CHANNEL_NAME,
      groupName: AICARDS_CLAUDE_GROUP_NAME,
      apiKeyEnvKeys: ["AICARDS_API_KEY_CLAUDE", "AICARDS_CLAUDE_API_KEY", "AICARDS_API_KEY"],
    };
  }
  return {
    family: "default",
    ...
    apiKeyEnvKeys: ["AICARDS_API_KEY_NON_CLAUDE", "AICARDS_API_KEY", "AICARDS_DEFAULT_API_KEY"],
  };
}

function getAicardsAuthProfiles() {
  const seen = new Set();
  return [
    getAicardsProfileForModel("deepseek-chat"),
    getAicardsProfileForModel("claude-sonnet-4-6"),
  ].map((profile) => {
    const apiKey = profile.apiKeyEnvKeys
      .map((key) => String(process.env[key] || "").trim())
      .find(isUsableSecret) || "";
    return { ...profile, apiKey };
  }).filter((profile) => {
    if (!profile.apiKey || seen.has(profile.providerKey)) return false;
    seen.add(profile.providerKey);
    return true;
  });
}

function normalizeAicardsCatalogItem(item = {}) {
  const actualModelId = String(item.model_name || item.modelName || item.model_id || item.id || item.model || item.name || "").trim();
  if (!actualModelId) return null;
  return {
    catalogId: String(item.id || "").trim(),
    actualModelId,
    displayName: String(item.display_name || item.displayName || actualModelId).trim(),
    modelType: String(item.model_type || item.modelType || detectModelCategory(actualModelId)).trim(),
    protocolType: String(item.protocol_type || item.protocolType || "").trim(),
    maxTokens: Number(item.max_tokens || item.maxTokens || 0),
    billingType: String(item.billing_type || item.billingType || "token").trim(),
    inputPrice: Number(item.input_price || item.inputPrice || 0),
    outputPrice: Number(item.output_price || item.outputPrice || 0),
    requestPrice: Number(item.request_price || item.requestPrice || 0),
    imageCreditMultiplier: Number(item.image_credit_multiplier || item.imageCreditMultiplier || 1),
    description: String(item.description || "").trim(),
    rawData: item,
  };
}

function aicardsCatalogAliases(item = {}) {
  return [item.actualModelId, item.displayName, item.catalogId]
    .filter(Boolean)
...
function pricePerMillionFromRaw(value) {
  const number = readPositiveNumber(value);
  if (!number) return 0;
  return number < 0.01 ? number * 1_000_000 : number;
}

function extractAicardsRawPricing(raw = {}) {
  const publicCatalog = raw.publicCatalog || raw.public_catalog || raw.catalog || raw.modelCatalog || {};
  const pricing = raw.pricing || raw.price || raw.cost || {};
  const currency = String(pricing.currency || raw.currency || raw.pricing_currency || publicCatalog.currency || "CNY").toUpperCase();
  const fx = currency === "CNY" || currency === "RMB"
    ? 1
    : safePositiveNumber(process.env.FLOWAPI_AICARDS_PRICE_FX_CNY, 7.25);
  const prompt = pricePerMillionFromRaw(
    pricing.prompt
      ?? pricing.input
      ?? pricing.input_cost
      ?? pricing.inputPrice
      ...
      ?? raw.input_price
      ?? raw.inputPrice
      ?? publicCatalog.input_price
      ?? publicCatalog.inputPrice
      ?? raw.inputCostPerMillion
      ?? raw.input_cost_per_million,
  );
  const completion = pricePerMillionFromRaw(
    pricing.completion
      ?? pricing.output
      ?? pricing.output_cost
      ?? pricing.outputPrice
      ...
      ?? raw.output_price
      ?? raw.outputPrice
      ?? publicCatalog.output_price
      ?? publicCatalog.outputPrice
      ?? raw.outputCostPerMillion
      ?? raw.output_cost_per_million,
  );
  const requestPrice = readPositiveNumber(
    raw.request_price
      ?? raw.requestPrice
      ?? pricing.request_price
      ?? pricing.requestPrice
      ?? publicCatalog.request_price
      ?? publicCatalog.requestPrice,
  );
  const billingType = String(
    pricing.billing_type
      ...
      ?? raw.billing_type
      ?? raw.billingType
      ?? publicCatalog.billing_type
      ?? publicCatalog.billingType
      ?? "token",
  ).toLowerCase();
  const imageCreditMultiplier = safePositiveNumber(
    raw.image_credit_multiplier
      ?? raw.imageCreditMultiplier
      ?? pricing.image_credit_multiplier
      ?? pricing.imageCreditMultiplier
      ?? publicCatalog.image_credit_multiplier
      ?? publicCatalog.imageCreditMultiplier,
    1,
  );
  if (prompt > 0 && completion > 0) {
    return {
      inputCost: roundPrice(prompt * fx),
      outputCost: roundPrice(completion * fx),
      source: "upstream_pricing",
      currency,
      fx,
      billingType,
      requestPrice,
      imageCreditMultiplier,
    };
  }
  if (readPositiveNumber(publicCatalog.input_price) > 0 || readPositiveNumber(publicCatalog.output_price) > 0) {
    return {
      inputCost: roundPrice(readPositiveNumber(publicCatalog.input_price) * fx),
      outputCost: roundPrice(readPositiveNumber(publicCatalog.output_price) * fx),
...
const ESTIMATED_COST_PRESETS_CNY_PER_MILLION = {
  haiku: { input: 8, output: 40 },
  sonnet: { input: 24, output: 120 },
  opus: { input: 120, output: 600 },
  claude: { input: 40, output: 200 },
  codex: { input: 24, output: 120 },
  gemini: { input: 8, output: 40 },
  deepseek: { input: 4, output: 18 },
  qwen: { input: 4, output: 18 },
  grok: { input: 20, output: 100 },
  image: { input: 80, output: 240 },
  chatgpt: { input: 40, output: 160 },
  general: { input: 30, output: 120 },
};

function estimatedCostPresetForModel(model = {}) {
  const id = String(model.actualModelId || model.actual_model_id || model.id || "").toLowerCase();
  const category = detectModelCategory(id);
  if (id.includes("haiku")) return ESTIMATED_COST_PRESETS_CNY_PER_MILLION.haiku;
  if (id.includes("sonnet")) return ESTIMATED_COST_PRESETS_CNY_PER_MILLION.sonnet;
  if (id.includes("opus")) return ESTIMATED_COST_PRESETS_CNY_PER_MILLION.opus;
  if (category === "image") return ESTIMATED_COST_PRESETS_CNY_PER_MILLION.image;
  return ESTIMATED_COST_PRESETS_CNY_PER_MILLION[category] || ESTIMATED_COST_PRESETS_CNY_PER_MILLION.general;
}

function buildEstimatedAicardsCost(model = {}) {
  if (process.env.FLOWAPI_AICARDS_ALLOW_ESTIMATED_COST !== "true") return null;
  const preset = estimatedCostPresetForModel(model);
  const multiplier = Math.max(1, safePositiveNumber(process.env.FLOWAPI_AICARDS_ESTIMATED_COST_MULTIPLIER, 1.6));
  return {
    inputCost: roundPrice(preset.input * multiplier),
    outputCost: roundPrice(preset.output * multiplier),
    source: "estimated_guardrail",
  };
}

function buildAicardsAutoPricing(model = {}, options = {}) {
  const minProfitMargin = Math.max(
    0.2,
    Number(options.minProfitMargin ?? process.env.FLOWAPI_AICARDS_AUTO_MIN_PROFIT_MARGIN ?? 0.45),
  );
  const sellMultiplier = Math.max(
    1 + minProfitMargin,
    safePositiveNumber(options.sellMultiplier || process.env.FLOWAPI_AICARDS_AUTO_SELL_MULTIPLIER, 2.2),
  );
  const rawPricing = extractAicardsRawPricing(model.rawData || model.raw_data || {});
  const existingCostSource = readPositiveNumber(model.inputCostPerMillion) > 0 && readPositiveNumber(model.outputCostPerMillion) > 0
    ? { inputCost: readPositiveNumber(model.inputCostPerMillion), outputCost: readPositiveNumber(model.outputCostPerMillion), source: "configured" }
    : null;
  const estimatedCost = buildEstimatedAicardsCost(model);
  const costSource = existingCostSource || rawPricing || estimatedCost;
  if (!costSource) {
    const error = new Error("缺少真实上游成本，已跳过自动发布；请在审核框填写成本和售价后再发布，或显式开启 FLOWAPI_AICARDS_ALLOW_ESTIMATED_COST=true 使用保守估算。");
    error.status = 400;
    throw error;
  }

  const inputCost = roundPrice(costSource.inputCost);
  const outputCost = roundPrice(costSource.outputCost);
  const existingInputSell = Number(model.sellInputPricePerMillion || 0);
  const existingOutputSell = Number(model.sellOutputPricePerMillion || 0);
  const inputSell = roundPrice(Math.max(existingInputSell, inputCost * sellMultiplier));
  const outputSell = roundPrice(Math.max(existingOutputSell, outputCost * sellMultiplier));
...
function normalizeModelItem(item = {}) {
  ...
  const actualModelId = String(raw.id || raw.model_name || raw.model || raw.name || raw.model_id || "").trim();
  if (!actualModelId) return null;
  const profile = getAicardsProfileForModel(actualModelId);
  const family = detectModelFamily(actualModelId);
  return {
    id: `aicards_${sanitizeId(actualModelId)}`,
    providerKey: raw.provider_key || raw.providerKey || profile.providerKey,
    upstreamChannel: raw.upstream_channel || raw.upstreamChannel || profile.channelName,
    channelName: raw.channel_name || raw.channelName || profile.channelName,
    groupName: raw.group_name || raw.groupName || profile.groupName,
    authFamily: profile.family,
    actualModelId,
    rawModelName: String(raw.display_name || raw.displayName || raw.name || raw.model || raw.model_name || actualModelId).trim(),
...
async function fetchAicardsPublicCatalog() {
  const url = "https://aicards.shop/api/public/models";
  ...
  if (!response.ok) {
    ...
    throw new Error(`备用上游公开模型与价格同步失败：HTTP ${response.status} ${sanitizeProviderError(text).slice(0, 200)}`);
  }
  const data = await response.json().catch(() => ({}));
  const list = Array.isArray(data?.data) ? data.data : Array.isArray(data?.models) ? data.models : Array.isArray(data) ? data : [];
  return list.map(normalizeAicardsCatalogItem).filter(Boolean);
}

function mapModelRow(row = {}) {
  const rawData = row.raw_data || {};
  const publicCatalog = rawData.publicCatalog || rawData.public_catalog || {};
  const publicPricing = rawData.publicPricing || rawData.public_pricing || {};
  return {
    id: row.id,
    providerKey: row.provider_key || row.provider || AICARDS_PROVIDER_KEY,
    upstreamChannel: row.upstream_channel || AICARDS_CHANNEL_NAME,
    channelName: row.channel_name || row.upstream_channel || AICARDS_CHANNEL_NAME,
    groupName: row.group_name || AICARDS_GROUP_NAME,
    ...
    publicCatalog,
    publicPricing,
    billingType: publicCatalog.billingType || rawData.billingType || "",
    requestPrice: Number(publicCatalog.requestPrice || rawData.requestPrice || 0),
    imageCreditMultiplier: Number(publicCatalog.imageCreditMultiplier || rawData.imageCreditMultiplier || 1),
  };
}

function looksLikeAicardsUpstream(upstream = {}) {
  let host = "";
  try {
    host = new URL(normalizeBaseUrl(upstream.baseUrl || "")).hostname.toLowerCase();
  } catch {
    host = "";
  }
  if (host === "aicards.shop") return true;
  const haystack = [
    upstream.name,
    upstream.type,
    upstream.remark,
  ].join(" ").toLowerCase();
  return haystack.includes("aicards");
}

async function getConfiguredAicardsUpstream() {
  if (!hasDatabase()) return null;
  const upstreams = await listUpstreams();
  const matches = upstreams.filter((item) => item.enabled !== false && looksLikeAicardsUpstream(item));
  if (matches.length > 1) {
    const error = new Error("检测到多个可用的 FlowAPI 备用线路配置，请在上游管理中只保留一条启用状态。");
    error.status = 409;
    throw error;
  }
  const matched = matches[0] || upstreams.find((item) => looksLikeAicardsUpstream(item));
  if (!matched?.id) return null;
  return getUpstream(matched.id, { includeSecret: true });
}

export async function getAicardsConfig() {
  const configured = await getConfiguredAicardsUpstream();
  const configuredBaseUrl = normalizeBaseUrl(configured?.baseUrl || "");
  const configuredApiKey = String(configured?.apiKey || "").trim();
  const envBaseUrl = normalizeBaseUrl(process.env.AICARDS_API_BASE_URL || process.env.AICARDS_BASE_URL || "");
  const authProfiles = getAicardsAuthProfiles();
  const envNonClaudeApiKey = String(
    process.env.AICARDS_API_KEY_NON_CLAUDE
    || process.env.AICARDS_API_KEY
    || process.env.AICARDS_DEFAULT_API_KEY
    || "",
  ).trim();
  const envClaudeApiKey = String(process.env.AICARDS_API_KEY_CLAUDE || process.env.AICARDS_CLAUDE_API_KEY || "").trim();
  const baseUrl = configuredBaseUrl || envBaseUrl;
  const apiKey = configuredApiKey || envNonClaudeApiKey || envClaudeApiKey;
  return {
    providerKey: AICARDS_PROVIDER_KEY,
    channelName: AICARDS_CHANNEL_NAME,
    groupName: AICARDS_GROUP_NAME,
    upstreamId: configured?.id || "",
    source: configuredBaseUrl && configuredApiKey ? "admin_upstream" : "environment",
    baseUrl,
    modelsUrl: `${baseUrl}/v1/models`,
    chatUrl: `${baseUrl}/v1/chat/completions`,
    apiKey,
    authProfiles,
    nonClaudeApiKey: authProfiles.find((profile) => profile.family === "default")?.apiKey || envNonClaudeApiKey,
    claudeApiKey: authProfiles.find((profile) => profile.family === "claude")?.apiKey || envClaudeApiKey,
    usernameConfigured: Boolean(process.env.AICARDS_USERNAME),
    passwordConfigured: Boolean(process.env.AICARDS_PASSWORD),
    enabled: Boolean(baseUrl && (authProfiles.some((profile) => isUsableSecret(profile.apiKey)) || isUsableSecret(apiKey))),
  };
}
...
async function fetchAicardsModels() {
  ...
  const profiles = getAicardsAuthProfiles();
  if (!profiles.length) throw new Error("AICARDS_API_KEY / AICARDS_API_KEY_CLAUDE 未配置或不可用");
  const [catalog, results] = await Promise.all([
    fetchAicardsPublicCatalog().catch(() => []),
    Promise.allSettled(profiles.map(async (profile) => {
      ...
      return list.map((item) => {
        ...
        return {
          ...
          upstreamChannel: profile.channelName,
  ...
  catalog.forEach((item) => {
    catalogIds.add(String(item.actualModelId || "").trim().toLowerCase());
    aicardsCatalogAliases(item).forEach((alias) => catalogMap.set(alias, item));
  });

  for (const catalogItem of catalog) {
    const profile = getAicardsProfileForModel(catalogItem.actualModelId);
    const authCandidates = results
      .filter((result) => result.status === "fulfilled")
      .flatMap((result) => result.value || [])
      .filter((model) => familyFromModelId(model.actualModelId || "") === profile.family);
    const authMatch = authCandidates.find((model) => aicardsCatalogAliases({
      actualModelId: model.actualModelId,
      displayName: model.rawModelName,
      catalogId: model.id,
    }).some((alias) => aicardsCatalogAliases(catalogItem).includes(alias)));
    const normalized = authMatch || normalizeModelItem({
      id: catalogItem.actualModelId,
      model_name: catalogItem.actualModelId,
      display_name: catalogItem.displayName,
      model_type: catalogItem.modelType,
    ...
    const model = {
      ...
      rawData: {
        ...(normalized.rawData || {}),
        publicCatalog: catalogItem,
        publicPricing: {
          inputPrice: catalogItem.inputPrice,
          outputPrice: catalogItem.outputPrice,
          requestPrice: catalogItem.requestPrice,
          billingType: catalogItem.billingType,
          imageCreditMultiplier: catalogItem.imageCreditMultiplier,
        },
      },
    };
    const key = `${model.providerKey}:${model.actualModelId}`;
  ...
  for (const result of results) {
    ...
    for (const model of result.value || []) {
      const key = `${model.providerKey}:${model.actualModelId}`;
      const actualKey = String(model.actualModelId || "").trim().toLowerCase();
      if (!model.actualModelId || seen.has(key) || catalogIds.has(actualKey)) continue;
      const catalogItem = catalogMap.get(String(model.actualModelId).trim().toLowerCase()) || catalogMap.get(String(model.rawModelName || "").trim().toLowerCase()) || null;
      seen.add(key);
      merged.push({
        ...model,
        rawData: {
  ...
  if (!merged.length) throw new Error("备用上游模型列表为空，请检查 AICARDS_API_KEY / AICARDS_API_KEY_CLAUDE");
  return merged;
}
  ...
  try {
    ...
    if (!hasDatabase()) {
      ...
      for (const model of models) {
        ...
        memory.channels.push({ id: channelId, ...model, baseUrl: config.baseUrl, isEnabled: false, providerKey: model.providerKey, channelName: model.channelName, groupName: model.groupName });
        memory.routeCandidates = memory.routeCandidates.filter((item) => item.id !== `rc_${channelId}`);
        memory.routeCandidates.push({ id: `rc_${channelId}`, publicModelId: model.publicModelId, channelId, actualModelId: model.actualModelId, providerKey: model.providerKey, isEnabled: false });
      }
    } else {
      for (const model of models) {
        const channelId = `aicards_${sanitizeId(model.publicModelId)}_${sanitizeId(model.actualModelId)}`;
          ...
          `INSERT INTO upstream_channels (
           ...
           ON CONFLICT (id) DO UPDATE SET
             ...
             route_strategy = EXCLUDED.route_strategy,
	             ...
	             updated_at = NOW()`,
          [channelId, model.providerKey, model.providerKey, model.channelName, config.baseUrl, model.actualModelId, model.publicModelId, model.groupName],
        );
        await query(
          ...
          `INSERT INTO route_candidates (
           ...
           ON CONFLICT (id) DO UPDATE SET
             ...
             weight = EXCLUDED.weight,
	             ...
	             updated_at = NOW()`,
          [`rc_${channelId}`, model.publicModelId, channelId, model.actualModelId, model.providerKey],
        );
      }
    }
    ...
    await writeDataSyncLog({
      ...
      rawSummary: {
        ...
        apiKeyConfigured: Boolean(config.authProfiles?.some((profile) => isUsableSecret(profile.apiKey))),
        claudeApiKeyConfigured: Boolean(config.claudeApiKey),
        nonClaudeApiKeyConfigured: Boolean(config.nonClaudeApiKey),
      },
    });
    return {
      ok: true,
...
export async function healthCheckAicards({ adminId = "", modelId = "", skipModelTest = false } = {}) {
  ...
  const profiles = getAicardsAuthProfiles();
  if (!profiles.length) throw new Error("AICARDS_API_KEY / AICARDS_API_KEY_CLAUDE 未配置或不可用");
  await assertSafeUpstreamUrl(`${config.baseUrl}/v1/models`);
  const started = Date.now();
  const response = await fetch(`${config.baseUrl}/v1/models`, {
    method: "GET",
    headers: {
      Authorization: `Bearer ${(profiles.find((profile) => profile.family === familyFromModelId(modelId || "")) || profiles[0]).apiKey}`,
      "Content-Type": "application/json",
    },
  ...
  if (modelsOk && selectedModel && !skipModelTest) {
    ...
    const selectedProfile = profiles.find((profile) => profile.family === familyFromModelId(selectedModel)) || profiles[0];
    await assertSafeUpstreamUrl(`${config.baseUrl}/v1/chat/completions`);
    try {
    ...
    await query(
      `UPDATE upstream_channels
       SET success_rate = $2,
           ...
           updated_at = NOW()
       WHERE (provider_key = ANY($1::text[]) OR provider_name = ANY($1::text[])) ${modelFilter}`,
      params,
    );
  }
  await writeDataSyncLog({
  ...
  return {
    ok,
    providerKey: modelId && familyFromModelId(modelId) === "claude" ? AICARDS_CLAUDE_PROVIDER_KEY : AICARDS_PROVIDER_KEY,
    channelName: modelId && familyFromModelId(modelId) === "claude" ? AICARDS_CLAUDE_CHANNEL_NAME : AICARDS_CHANNEL_NAME,
...
export async function listAicardsSyncedModels() {
  await ensureAicardsProviderSchema();
  if (!hasDatabase()) {
    return memory.upstreamModels.map((item) => {
  ...
  const result = await query(
    `SELECT
       ...
       channels.group_name AS group_name,
     ...
     WHERE models.provider_key = ANY($1::text[]) OR models.provider = ANY($1::text[]) OR models.upstream_channel = ANY($2::text[])
     ORDER BY models.last_synced_at DESC, models.actual_model_id ASC
     LIMIT 500`,
    [AICARDS_PROVIDER_KEYS, AICARDS_CHANNEL_NAMES],
  );
...
export async function reviewAicardsCandidate(input = {}, adminId = "") {
  ...
  const profile = getAicardsProfileForModel(actualModelId || publicModelId);
  ...
  if (!hasDatabase()) {
    ...
    if (enable || publish) {
      const error = new Error("无数据库模式不能启用或发布备用线路，请先配置生产数据库并通过健康检查。");
      error.status = 400;
      throw error;
    }
    ...
    `SELECT models.*, channels.id AS existing_channel_id, channels.success_rate, channels.last_health_check_at
     ...
     LEFT JOIN upstream_channels channels ON channels.id = models.channel_id
      WHERE (models.id = $1 OR models.actual_model_id = $2)
       AND (models.provider_key = ANY($3::text[]) OR models.provider = ANY($3::text[]) OR models.upstream_channel = ANY($4::text[]))
     ORDER BY models.last_synced_at DESC
     LIMIT 1`,
    [id, actualModelId, AICARDS_PROVIDER_KEYS, AICARDS_CHANNEL_NAMES],
  );
  ...
  await query(
    `INSERT INTO route_candidates (
     ...
     ON CONFLICT (id) DO UPDATE SET
       ...
       is_enabled = EXCLUDED.is_enabled,
       updated_at = NOW()`,
    [`rc_${channelId}`, publicModelId, channelId, model.actual_model_id, profile.providerKey, Number(input.priority || 90), enable],
  );
...
function validateBulkPublishCandidate(model = {}) {
  const inputCost = assertPositivePrice(model.inputCostPerMillion, `${model.publicModelId} 输入上游成本`);
  const outputCost = assertPositivePrice(model.outputCostPerMillion, `${model.publicModelId} 输出上游成本`);
  const inputSell = assertPositivePrice(model.sellInputPricePerMillion, `${model.publicModelId} 输入销售价`);
  const outputSell = assertPositivePrice(model.sellOutputPricePerMillion, `${model.publicModelId} 输出销售价`);
  const minProfitMargin = Number(model.minProfitMargin || 0.2);
  assertMargin({ inputCost, outputCost, inputSell, outputSell, minProfitMargin });
  assertSafePublicModelText(model.publicModelId, "FlowAPI 对外模型 ID");
  assertSafePublicModelText(model.displayName, "用户看到的模型名称");
  if (Number(model.channelSuccessRate || 0) <= 0 || !model.channelLastHealthCheckAt) {
    const error = new Error("未通过健康检查");
    error.status = 400;
    throw error;
  }
  return { inputCost, outputCost, inputSell, outputSell, minProfitMargin };
}

export async function bulkPublishAicardsCandidates({
  modelIds = [],
  adminId = "",
  maxCount = 50,
  autoPrice = false,
  ...
  await ensureAicardsProviderSchema();
  if (!hasDatabase()) {
    const error = new Error("无数据库模式不能批量发布备用线路，请先配置生产数据库。");
    error.status = 400;
    throw error;
  }

  if (autoSync) {
    await syncAicardsModels({ adminId });
  }
  if (autoHealthCheck) {
    await healthCheckAicards({ adminId, skipModelTest: true });
  }

  const requestedIds = new Set((Array.isArray(modelIds) ? modelIds : String(modelIds || "").split(",")).map((item) => String(item || "").trim()).filter(Boolean));
  const candidates = await listAicardsSyncedModels();
  const targets = candidates
    .filter((model) => !requestedIds.size || requestedIds.has(model.id) || requestedIds.has(model.publicModelId) || requestedIds.has(model.actualModelId))
    .slice(0, Math.max(1, Math.min(Number(maxCount || 50), 200)));

  if (!targets.length) {
    const error = new Error("没有找到可发布的备用线路候选，请先同步模型。");
    error.status = 404;
    throw error;
  }
  ...
  for (const model of targets) {
    try {
      if (!includeImages && (model.category === "image" || /image|video|audio|embedding/i.test(model.actualModelId || ""))) {
        const error = new Error("非文本模型已跳过，请到生成图片/视频等专用后台单独开通。");
        error.status = 400;
        throw error;
      }
      if (autoHealthCheck && perModelHealthCheck) {
        const health = await healthCheckAicards({ adminId, modelId: model.actualModelId, skipModelTest: false });
        if (!health.ok) {
          const error = new Error(health.lastError || "当前模型轻量检测失败，已跳过");
          error.status = 400;
          throw error;
        }
      }
      const price = autoPrice
        ? buildAicardsAutoPricing(model, { minProfitMargin, sellMultiplier })
        : validateBulkPublishCandidate(model);
      if (!autoPrice) validateBulkPublishCandidate(model);
      const result = await reviewAicardsCandidate({
        id: model.id,
        actualModelId: model.actualModelId,
        publicModelId: model.publicModelId,
        displayName: model.displayName || model.rawModelName || model.publicModelId,
        modelType: model.category === "image" ? "image" : "text",
        inputCostPerMillion: price.inputCost,
        outputCostPerMillion: price.outputCost,
        sellInputPricePerMillion: price.inputSell,
        sellOutputPricePerMillion: price.outputSell,
        minProfitMargin: price.minProfitMargin,
      ...
      published.push({
        id: model.id,
        publicModelId: result.publicModelId,
        displayName: model.displayName,
        actualModelId: model.actualModelId,
        pricingSource: price.source || "configured",
        sellInputPricePerMillion: price.inputSell,
        sellOutputPricePerMillion: price.outputSell,
      });
    } catch (error) {
      skipped.push({
        ...
        publicModelId: model.publicModelId,
        displayName: model.displayName,
        reason: sanitizeProviderError(error.message || "不满足发布条件"),
      });
    }
  }
  ...
  await writeDataSyncLog({
    adminId,
    action: "bulk_publish",
    status: published.length ? "success" : "failed",
    totalItems: targets.length,
    addedCount: published.length,
    updatedCount: 0,
    errorMessage: published.length ? "" : "没有候选满足批量发布条件",
    rawSummary: {
