...
import { getUpstream, listPublishedModels, listUpstreams, upsertPublishedModel } from "./admin-commercial-config";
...
function inferAicardsCostPreset(model = {}) {
  const id = String(model.actualModelId || model.publicModelId || model.displayName || "").toLowerCase();
  const category = model.category || detectModelCategory(id);

  if (category === "image" || id.includes("video") || id.includes("audio") || id.includes("embedding")) {
    return null;
  }

  if (id.includes("opus")) return { inputCost: 110, outputCost: 560 };
  if (id.includes("sonnet")) return { inputCost: 32, outputCost: 165 };
  if (id.includes("haiku")) return { inputCost: 8, outputCost: 40 };
  if (id.includes("gpt-5.5") || id.includes("gpt-5-5")) return { inputCost: 42, outputCost: 240 };
  if (id.includes("gpt-5.4") || id.includes("gpt-5-4")) {
    if (id.includes("mini")) return { inputCost: 8, outputCost: 48 };
    return { inputCost: 32, outputCost: 190 };
  }
  if (id.includes("gpt")) return { inputCost: 24, outputCost: 120 };
  if (id.includes("codex")) return { inputCost: 36, outputCost: 180 };
  if (id.includes("gemini")) {
    if (id.includes("flash")) return { inputCost: 5, outputCost: 28 };
    return { inputCost: 24, outputCost: 140 };
  }
  if (id.includes("grok")) return { inputCost: 28, outputCost: 150 };
  if (id.includes("deepseek")) return { inputCost: 4, outputCost: 18 };
  if (id.includes("qwen")) return { inputCost: 3, outputCost: 14 };
  return { inputCost: 24, outputCost: 120 };
}
...
function buildAicardsAutoPricing(model = {}, options = {}) {
  ...
  const sellMultiplier = Math.max(
    1 + minProfitMargin,
    readPositiveEnvNumber("FLOWAPI_AICARDS_AUTO_SELL_MULTIPLIER", Number(options.sellMultiplier || 2.2)),
  );
  const preset = inferAicardsCostPreset(model);
  if (!preset) {
    const error = new Error("图片、视频、音频类模型请在对应功能页单独开通，避免误按文本模型计费。");
    error.status = 400;
    throw error;
  }

  const configuredInputCost = readPositiveEnvNumber("FLOWAPI_AICARDS_DEFAULT_INPUT_COST_CNY_PER_M", 0);
  const configuredOutputCost = readPositiveEnvNumber("FLOWAPI_AICARDS_DEFAULT_OUTPUT_COST_CNY_PER_M", 0);
  const inputCost = roundPrice(Number(model.inputCostPerMillion || 0) || configuredInputCost || preset.inputCost);
  const outputCost = roundPrice(Number(model.outputCostPerMillion || 0) || configuredOutputCost || preset.outputCost);
  ...
  return {
    ...
    minProfitMargin,
    sellMultiplier,
    source: Number(model.inputCostPerMillion || 0) > 0 && Number(model.outputCostPerMillion || 0) > 0
      ? "configured"
