...
function normalizeEmailFrom(value = "") {
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(address)) {
    return "FlowAPI <hello@flowapi.fun>";
  }
  const displayName = raw.includes("<") && raw.includes(">")
    ? raw.replace(/<[^>]*>/g, "").trim()
    : raw;
  const safeDisplay = displayName && !displayName.includes("@") ? displayName : "FlowAPI";
  return `${safeDisplay} <${address}>`;
}

export function getSafeEmailFrom(value = EMAIL_FROM) {
  return normalizeEmailFrom(value);
}

const SAFE_EMAIL_FROM = normalizeEmailFrom(EMAIL_FROM);
const SAFE_SMTP_FROM = normalizeEmailFrom(SMTP_FROM);
...
async function sendViaSmtp({ to, subject, html }) {
  ...
  const raw = buildRawMessage({ from: SAFE_SMTP_FROM, to, subject, html });
